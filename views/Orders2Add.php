...
$Page->showMessage();
?>
<form name="forders2add" id="forders2add" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_orders2_CustomerID">
    <select
        ...
        data-select2-id="forders2add_x_CustomerID"
        <?php } ?>
        data-table="orders2"
...
<?php if (!$Page->CustomerID->IsNativeSelect) { ?>
<script>
loadjs.ready("forders2add", function() {
    var options = { name: "x_CustomerID", selectId: "forders2add_x_CustomerID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (forders2add.lists.CustomerID?.lookupOptions.length) {
        options.data = { id: "x_CustomerID", form: "forders2add" };
    } else {
        options.ajax = { id: "x_CustomerID", form: "forders2add", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_orders2_EmployeeID">
    <select
        ...
        data-select2-id="forders2add_x_EmployeeID"
        <?php } ?>
        data-table="orders2"
...
<script>
loadjs.ready("forders2add", function() {
    var options = { name: "x_EmployeeID", selectId: "forders2add_x_EmployeeID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (forders2add.lists.EmployeeID?.lookupOptions.length) {
        options.data = { id: "x_EmployeeID", form: "forders2add" };
    } else {
        options.ajax = { id: "x_EmployeeID", form: "forders2add", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="forders2add"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="forders2add" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
