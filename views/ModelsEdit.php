...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fmodelsedit" id="fmodelsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->getCurrentMasterTable() == "trademarks") { ?>
...
<?php if ($Page->Trademark->getSessionValue() != "") { ?>
...
<input type="hidden" id="x_Trademark" name="x_Trademark" value="<?= HtmlEncode($Page->Trademark->CurrentValue) ?>" data-hidden="1">
<?php } else { ?>
