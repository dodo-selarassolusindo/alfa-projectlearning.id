...
$Page->showMessage();
?>
<form name="fcarsadd" id="fcarsadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fcarsadd_x_Trademark"
        <?php } ?>
        data-table="cars"
...
<?php if (!$Page->Trademark->IsNativeSelect) { ?>
<script>
loadjs.ready("fcarsadd", function() {
    var options = { name: "x_Trademark", selectId: "fcarsadd_x_Trademark" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcarsadd.lists.Trademark?.lookupOptions.length) {
        options.data = { id: "x_Trademark", form: "fcarsadd" };
    } else {
        options.ajax = { id: "x_Trademark", form: "fcarsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group flex-nowrap">
    <select
        ...
        data-select2-id="fcarsadd_x_Model"
        <?php } ?>
        data-table="cars"
        data-field="x_Model"
        data-page="0"
...
<script>
loadjs.ready("fcarsadd", function() {
    var options = { name: "x_Model", selectId: "fcarsadd_x_Model" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcarsadd.lists.Model?.lookupOptions.length) {
        options.data = { id: "x_Model", form: "fcarsadd" };
    } else {
        options.ajax = { id: "x_Model", form: "fcarsadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<div class="ew-nav<?= $Page->MultiPages->containerClasses() ?>" id="pages_CarsAdd"><!-- multi-page tabs -->
...
<i class="fa-regular fa-square<?php if (ConvertToBool($Page->TransmissAutomatic->CurrentValue)) { ?>-check<?php } ?> ew-icon ew-boolean"></i>
</span>
...
<script>
loadjs.ready(["fcarsadd", "editor"], function() {
    ew.createEditor("fcarsadd", "x_Description", 35, 4, <?= $Page->Description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
<?php } else { ?>
...
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcarsadd" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcarsadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcarsadd"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="fcarsadd" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
