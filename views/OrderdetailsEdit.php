...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="forderdetailsedit" id="forderdetailsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { orderdetails: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var forderdetailsedit;
...
<?php } ?>
<input type="hidden" data-table="orderdetails" data-field="x_OrderID" data-hidden="1" data-old name="o_OrderID" id="o_OrderID" value="<?= HtmlEncode($Page->OrderID->OldValue ?? $Page->OrderID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_orderdetails_ProductID">
    <select
        ...
        data-select2-id="forderdetailsedit_x_ProductID"
...
<script>
loadjs.ready("forderdetailsedit", function() {
    var options = { name: "x_ProductID", selectId: "forderdetailsedit_x_ProductID" };
    if (forderdetailsedit.lists.ProductID?.lookupOptions.length) {
        options.data = { id: "x_ProductID", form: "forderdetailsedit" };
    } else {
        options.ajax = { id: "x_ProductID", form: "forderdetailsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options = Object.assign({}, ew.modalLookupOptions, options, ew.vars.tables.orderdetails.fields.ProductID.modalLookupOptions);
    ew.createModalLookup(options);
});
</script>
<input type="hidden" data-table="orderdetails" data-field="x_ProductID" data-hidden="1" data-old name="o_ProductID" id="o_ProductID" value="<?= HtmlEncode($Page->ProductID->OldValue ?? $Page->ProductID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="forderdetailsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="forderdetailsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
