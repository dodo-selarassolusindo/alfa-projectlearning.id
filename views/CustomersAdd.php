...
<script>
loadjs.ready("fcustomersadd", function() {
    fcustomersadd.createAutoSuggest(Object.assign({"id":"x_CustomerID","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->CustomerID->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.customers.fields.CustomerID.autoSuggestOptions));
});
</script>
...
<?php
    if (in_array("orders", explode(",", $Page->getCurrentDetailTable())) && $orders->DetailAdd) {
?>
