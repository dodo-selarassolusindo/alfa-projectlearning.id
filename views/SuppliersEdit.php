...
<?php if ($Page->SupplierID->Visible) { // SupplierID ?>
    <div id="r_SupplierID"<?= $Page->SupplierID->rowAttributes() ?>>
        <label id="elh_suppliers_SupplierID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->SupplierID->caption() ?><?= $Page->SupplierID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="suppliers" data-field="x_SupplierID" data-hidden="1" name="x_SupplierID" id="x_SupplierID" value="<?= HtmlEncode($Page->SupplierID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsuppliersedit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="fsuppliersedit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
