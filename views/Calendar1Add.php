...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        .setFields([
            ...
            ["End", [fields.End.visible && fields.End.required ? ew.Validators.required(fields.End.caption) : null, ew.Validators.datetime(fields.End.clientFormatPattern), ew.Validators.minDate(() => fCalendar1add.getFieldElements("Start"))], fields.End.isInvalid],
...
$Page->showMessage();
?>
<form name="fCalendar1add" id="fCalendar1add" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
loadjs.ready("fCalendar1add", function() {
    fCalendar1add.createAutoSuggest(Object.assign({"id":"x_ClassNames","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->ClassNames->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.Calendar1.fields.ClassNames.autoSuggestOptions));
});
</script>
</span>
</div></div>
...
<span id="el_Calendar1_Display">
    <select
        ...
        data-select2-id="fCalendar1add_x_Display"
        <?php } ?>
...
<script>
loadjs.ready("fCalendar1add", function() {
    var options = { name: "x_Display", selectId: "fCalendar1add_x_Display" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fCalendar1add.lists.Display?.lookupOptions.length) {
        options.data = { id: "x_Display", form: "fCalendar1add" };
    } else {
        options.ajax = { id: "x_Display", form: "fCalendar1add", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fCalendar1add"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fCalendar1add" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
