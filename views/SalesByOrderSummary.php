...
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<form name="fSales_By_Ordersrch" id="fSales_By_Ordersrch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
...
<?php if ($Page->OrderID->Visible) { ?>
    ...
    <th data-name="OrderID"<?= $Page->OrderID->cellAttributes("ew-rpt-grp-caret") ?>><?= $Page->OrderID->groupToggleIcon() ?></th>
    <?php } else { ?>
