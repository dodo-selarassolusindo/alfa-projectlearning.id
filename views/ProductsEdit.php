...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fproductsedit" id="fproductsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->ProductID->Visible) { // ProductID ?>
    <div id="r_ProductID"<?= $Page->ProductID->rowAttributes() ?>>
        <label id="elh_products_ProductID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->ProductID->caption() ?><?= $Page->ProductID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_products_SupplierID">
    <select
        ...
        data-select2-id="fproductsedit_x_SupplierID"
        data-table="products"
...
<script>
loadjs.ready("fproductsedit", function() {
    var options = { name: "x_SupplierID", selectId: "fproductsedit_x_SupplierID" };
    if (fproductsedit.lists.SupplierID?.lookupOptions.length) {
        options.data = { id: "x_SupplierID", form: "fproductsedit" };
    } else {
        options.ajax = { id: "x_SupplierID", form: "fproductsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_products_CategoryID">
    <select
        ...
        data-select2-id="fproductsedit_x_CategoryID"
        <?php } ?>
        data-table="products"
...
<script>
loadjs.ready("fproductsedit", function() {
    var options = { name: "x_CategoryID", selectId: "fproductsedit_x_CategoryID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fproductsedit.lists.CategoryID?.lookupOptions.length) {
        options.data = { id: "x_CategoryID", form: "fproductsedit" };
    } else {
        options.ajax = { id: "x_CategoryID", form: "fproductsedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fproductsedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fproductsedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
