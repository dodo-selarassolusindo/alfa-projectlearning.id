...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fordersedit" id="fordersedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<input type="hidden" data-table="orders" data-field="x_OrderID" data-hidden="1" name="x_OrderID" id="x_OrderID" value="<?= HtmlEncode($Page->OrderID->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_orders_CustomerID">
    <select
        ...
        data-select2-id="fordersedit_x_CustomerID"
        data-table="orders"
...
loadjs.ready("fordersedit", function() {
    var options = { name: "x_CustomerID", selectId: "fordersedit_x_CustomerID" };
    if (fordersedit.lists.CustomerID?.lookupOptions.length) {
        options.data = { id: "x_CustomerID", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_CustomerID", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_orders_EmployeeID">
    <select
        ...
        data-select2-id="fordersedit_x_EmployeeID"
        <?php } ?>
        data-table="orders"
...
<script>
loadjs.ready("fordersedit", function() {
    var options = { name: "x_EmployeeID", selectId: "fordersedit_x_EmployeeID" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fordersedit.lists.EmployeeID?.lookupOptions.length) {
        options.data = { id: "x_EmployeeID", form: "fordersedit" };
    } else {
        options.ajax = { id: "x_EmployeeID", form: "fordersedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fordersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fordersedit", "x_RequiredDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fordersedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fordersedit", "x_ShippedDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php
    if (in_array("orderdetails", explode(",", $Page->getCurrentDetailTable())) && $orderdetails->DetailEdit) {
?>
...
?>
<?php
    if (in_array("order_details_extended", explode(",", $Page->getCurrentDetailTable())) && $order_details_extended->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fordersedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fordersedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
