...
$Page->showMessage();
?>
<form name="femployeesadd" id="femployeesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_employees_TitleOfCourtesy">
    <select
        ...
        data-select2-id="femployeesadd_x_TitleOfCourtesy"
        <?php } ?>
...
loadjs.ready("femployeesadd", function() {
    var options = { name: "x_TitleOfCourtesy", selectId: "femployeesadd_x_TitleOfCourtesy" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femployeesadd.lists.TitleOfCourtesy?.lookupOptions.length) {
        options.data = { id: "x_TitleOfCourtesy", form: "femployeesadd" };
    } else {
        options.ajax = { id: "x_TitleOfCourtesy", form: "femployeesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["femployeesadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("femployeesadd", "x_BirthDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["femployeesadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("femployeesadd", "x_HireDate", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["femployeesadd", "editor"], function() {
    ew.createEditor("femployeesadd", "x_Notes", 35, 4, <?= $Page->Notes->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<span id="el_employees_ReportsTo">
    <select
        ...
        data-select2-id="femployeesadd_x_ReportsTo"
        <?php } ?>
        data-table="employees"
...
<script>
loadjs.ready("femployeesadd", function() {
    var options = { name: "x_ReportsTo", selectId: "femployeesadd_x_ReportsTo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femployeesadd.lists.ReportsTo?.lookupOptions.length) {
        options.data = { id: "x_ReportsTo", form: "femployeesadd" };
    } else {
        options.ajax = { id: "x_ReportsTo", form: "femployeesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_employees__UserLevel">
    <select
        ...
        data-select2-id="femployeesadd_x__UserLevel"
        <?php } ?>
        data-table="employees"
...
<script>
loadjs.ready("femployeesadd", function() {
    var options = { name: "x__UserLevel", selectId: "femployeesadd_x__UserLevel" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (femployeesadd.lists._UserLevel?.lookupOptions.length) {
        options.data = { id: "x__UserLevel", form: "femployeesadd" };
    } else {
        options.ajax = { id: "x__UserLevel", form: "femployeesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="femployeesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="femployeesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
