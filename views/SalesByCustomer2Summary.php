...
<?php if (!$Page->isExport() && !$Page->DrillDown && !$DashboardReport) { ?>
<form name="fSales_By_Customer_2srch" id="fSales_By_Customer_2srch" class="ew-form ew-ext-search-form" action="<?= CurrentPageUrl(false) ?>" novalidate autocomplete="off">
...
<?php if ($Page->CompanyName->Visible) { ?>
    ...
    <th data-name="CompanyName"<?= $Page->CompanyName->cellAttributes("ew-rpt-grp-caret") ?>><?= $Page->CompanyName->groupToggleIcon() ?></th>
    <?php } else { ?>
...
<?php if ($Page->CompanyName->Visible) { ?>
        <td data-field="CompanyName"<?= $Page->CompanyName->cellAttributes("ew-rpt-grp-caret") ?>><?= $Page->CompanyName->groupToggleIcon() ?></td>
<?php } ?>
...
<?php } ?>
<?php if ($Page->OrderID->Visible) { ?>
        <td data-field="OrderID"<?= $Page->OrderID->cellAttributes("ew-rpt-grp-caret") ?>><?= $Page->OrderID->groupToggleIcon() ?></td>
<?php } ?>
...
<?php if ($Page->TotalGroups > 0) { ?>
<?php
    $Page->Quantity->getSum($Page->OrderID->Records, false); // Get Sum
    $Page->ExtendedPrice->getSum($Page->OrderID->Records, false); // Get Sum
    $Page->resetAttributes();
    $Page->RowType = RowType::TOTAL;
    $Page->RowTotalType = RowSummary::GROUP;
