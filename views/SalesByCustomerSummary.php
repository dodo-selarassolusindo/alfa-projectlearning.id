...
<?php if ($Page->TotalGroups > 0) { ?>
<?php
    $Page->Quantity->getAvg($Page->OrderID->Records, false); // Get Avg
...
<?php if ($Page->TotalGroups > 0) { ?>
<?php
    $Page->Quantity->getAvg($Page->CompanyName->Records, false); // Get Avg
    $Page->ExtendedPrice->getSum($Page->CompanyName->Records, false); // Get Sum
    $Page->resetAttributes();
    $Page->RowType = RowType::TOTAL;
    $Page->RowTotalType = RowSummary::GROUP;
    $Page->RowTotalSubType = RowTotal::FOOTER;
