...
class SuppliersList extends Suppliers
{
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fsupplierssrch", $filters);
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_suppliers", "data-rowtype" => RowType::ADD]);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['ContactName'])) { // ContactName
            $this->ContactName->setFormValue($row['ContactName']);
        }
        if (isset($row['ContactTitle'])) { // ContactTitle
            $this->ContactTitle->setFormValue($row['ContactTitle']);
        }
    }
