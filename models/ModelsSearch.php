...
class ModelsSearch extends Models
{
    ...
    // Page object name
    public $PageObjName = "ModelsSearch";
    ...
    // CSS class/style
    public $CurrentPageName = "modelssearch";
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $curVal = trim(strval($this->Trademark->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->Trademark->AdvancedSearch->ViewValue = $this->Trademark->lookupCacheOption($curVal);
            } else {
                $this->Trademark->AdvancedSearch->ViewValue = $this->Trademark->Lookup !== null && is_array($this->Trademark->lookupOptions()) && count($this->Trademark->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->Trademark->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->Trademark->EditValue = array_values($this->Trademark->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->Trademark->Lookup->getTable()->Fields["ID"]->searchExpression(), "=", $this->Trademark->AdvancedSearch->SearchValue, $this->Trademark->Lookup->getTable()->Fields["ID"]->searchDataType(), "");
                }
