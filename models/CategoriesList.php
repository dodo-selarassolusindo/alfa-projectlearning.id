 ...
 * Page class
 */
class CategoriesList extends Categories
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CategoriesList";
    ...
    public $FormName = "fcategorieslist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "categorieslist";
    ...
    public function setVisibility()
    {
        ...
        $this->Icon_17->Visible = false;
        $this->Icon_25->Visible = false;
    }
    ...
    public function __construct()
    {
        ...
        $this->MultiDeleteUrl = "categoriesdelete";
        $this->MultiUpdateUrl = "categoriesupdate";
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->CategoryName->AdvancedSearch->toJson(), ","); // Field CategoryName
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcategoriessrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field CategoryName
        $this->CategoryName->AdvancedSearch->SearchValue = @$filter["x_CategoryName"];
        $this->CategoryName->AdvancedSearch->SearchOperator = @$filter["z_CategoryName"];
        $this->CategoryName->AdvancedSearch->SearchCondition = @$filter["v_CategoryName"];
        $this->CategoryName->AdvancedSearch->SearchValue2 = @$filter["y_CategoryName"];
        $this->CategoryName->AdvancedSearch->SearchOperator2 = @$filter["w_CategoryName"];
        $this->CategoryName->AdvancedSearch->save();
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->CategoryName, $default, false); // CategoryName
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->CategoryName->AdvancedSearch->save(); // CategoryName
    ...
    public function showFilterList()
    {
        ...
        // Field CategoryName
        $filter = $this->queryBuilderWhere("CategoryName");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->CategoryName, false, false);
        }
        if ($filter != "") {
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->CategoryName->AdvancedSearch->issetSession()) {
            return true;
        }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->CategoryName->AdvancedSearch->unsetSession();
    ...
    protected function restoreSearchParms()
    {
        ...
        $this->CategoryName->AdvancedSearch->load();
    ...
    protected function setupListOptions()
    {
        ...
        $item->CssClass = "text-nowrap";
        $item->Visible = $Security->allowList(CurrentProjectID() . 'products');
        $item->OnLeft = true;
    ...
    public function renderListOptions()
    {
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"categories\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcategorieslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcategorieslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        if ($Security->allowList(CurrentProjectID() . 'products')) {
            ...
            $body = "<a class=\"btn btn-default ew-row-link ew-detail" . ($this->ListOptions->UseDropDownButton ? " dropdown-toggle" : "") . "\" data-action=\"list\" href=\"" . HtmlEncode("productslist?" . Config("TABLE_SHOW_MASTER") . "=categories&" . GetForeignKeyUrl("fk_CategoryID", $this->CategoryID->CurrentValue) . "") . "\">" . $body . "</a>";
            $links = "";
        ...
        $opt->Body = "<div class=\"form-check\"><input type=\"checkbox\" id=\"key_m_" . $this->RowCount . "\" name=\"key_m[]\" class=\"form-check-input ew-multi-select\" value=\"" . HtmlEncode($this->CategoryID->CurrentValue) . "\" data-ew-action=\"select-key\"></div>";
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "CategoryName");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fcategoriessrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fcategoriessrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcategorieslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_categories", "data-rowtype" => RowType::ADD]);
        ...
        $this->RowAttrs->merge([
            ...
            "id" => "r" . $this->RowCount . "_categories",
    ...
    protected function loadSearchValues()
    {
        ...
        // CategoryName
        if ($this->CategoryName->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->CategoryName->AdvancedSearch->SearchValue != "" || $this->CategoryName->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->CategoryID->EditValue = $this->CategoryID->AdvancedSearch->SearchValue;
            ...
            if (!$this->CategoryName->Raw) {
                $this->CategoryName->AdvancedSearch->SearchValue = HtmlDecode($this->CategoryName->AdvancedSearch->SearchValue);
            }
            $this->CategoryName->EditValue = HtmlEncode($this->CategoryName->AdvancedSearch->SearchValue);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcategorieslist" data-ew-action="email" data-custom="false" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fcategoriessrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-table=\"categories\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-ew-action=\"modal\" data-url=\"categoriessearch\" data-btn=\"SearchBtn\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-advanced-search\" title=\"" . $Language->phrase("AdvancedSearch", true) . "\" data-caption=\"" . $Language->phrase("AdvancedSearch", true) . "\" href=\"categoriessearch\">" . $Language->phrase("AdvancedSearch", false) . "</a>";
        }
        $item->Visible = true;
