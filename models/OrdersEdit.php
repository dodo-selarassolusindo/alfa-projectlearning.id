...
class OrdersEdit extends Orders
{
    ...
    protected function validateForm()
    {
        ...
        if (in_array("orderdetails", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
        $detailPage = Container("OrderDetailsExtendedGrid");
        if (in_array("order_details_extended", $detailTblVar) && $detailPage->DetailEdit) {
            $detailPage->run();
            $validateForm = $validateForm && $detailPage->validateGridForm();
        }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("OrderdetailsGrid");
            if (in_array("orderdetails", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
                $Security->loadCurrentUserLevel($this->ProjectID . "orderdetails"); // Load user level of detail table
                $editRow = $detailPage->gridUpdate();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
            }
            $detailPage = Container("OrderDetailsExtendedGrid");
            if (in_array("order_details_extended", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
