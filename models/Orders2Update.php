...
class Orders2Update extends Orders2
{
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["orders2"]) || $GLOBALS["orders2"]::class == PROJECT_NAMESPACE . "orders2") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (
                    ...
                    if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                        ...
                        $result["view"] = SameString($pageName, "orders2view"); // If View page, no primary button
                    } else { // List page
        ...
        if ($rs = $this->loadRecordset()) {
            ...
            while ($row = $rs->fetch()) {
                if ($i == 1) {
                    ...
                    if (!CompareValue($this->OrderID->DbValue, $row['OrderID'])) {
                        $this->OrderID->CurrentValue = null;
                    }
                    if (!CompareValue($this->CustomerID->DbValue, $row['CustomerID'])) {
                        $this->CustomerID->CurrentValue = null;
                    }
                    if (!CompareValue($this->EmployeeID->DbValue, $row['EmployeeID'])) {
                        $this->EmployeeID->CurrentValue = null;
                    }
                    if (!CompareValue($this->OrderDate->DbValue, $row['OrderDate'])) {
                        $this->OrderDate->CurrentValue = null;
                    }
                    if (!CompareValue($this->RequiredDate->DbValue, $row['RequiredDate'])) {
                        $this->RequiredDate->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShippedDate->DbValue, $row['ShippedDate'])) {
                        $this->ShippedDate->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipVia->DbValue, $row['ShipVia'])) {
                        $this->ShipVia->CurrentValue = null;
                    }
                    if (!CompareValue($this->Freight->DbValue, $row['Freight'])) {
                        $this->Freight->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipName->DbValue, $row['ShipName'])) {
                        $this->ShipName->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipAddress->DbValue, $row['ShipAddress'])) {
                        $this->ShipAddress->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipCity->DbValue, $row['ShipCity'])) {
                        $this->ShipCity->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipRegion->DbValue, $row['ShipRegion'])) {
                        $this->ShipRegion->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipPostalCode->DbValue, $row['ShipPostalCode'])) {
                        $this->ShipPostalCode->CurrentValue = null;
                    }
                    if (!CompareValue($this->ShipCountry->DbValue, $row['ShipCountry'])) {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->CustomerID->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->CustomerID->Lookup->getTable()->Fields["CustomerID"]->searchExpression(), "=", $this->CustomerID->CurrentValue, $this->CustomerID->Lookup->getTable()->Fields["CustomerID"]->searchDataType(), "");
                }
            ...
            if ($this->EmployeeID->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->EmployeeID->Lookup->getTable()->Fields["EmployeeID"]->searchExpression(), "=", $this->EmployeeID->CurrentValue, $this->EmployeeID->Lookup->getTable()->Fields["EmployeeID"]->searchDataType(), "");
                }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['EmployeeID'])) { // EmployeeID
            $this->EmployeeID->CurrentValue = $row['EmployeeID'];
        }
        if (isset($row['OrderDate'])) { // OrderDate
            $this->OrderDate->CurrentValue = $row['OrderDate'];
        }
        if (isset($row['RequiredDate'])) { // RequiredDate
            $this->RequiredDate->CurrentValue = $row['RequiredDate'];
        }
        if (isset($row['ShippedDate'])) { // ShippedDate
            $this->ShippedDate->CurrentValue = $row['ShippedDate'];
        }
        if (isset($row['ShipVia'])) { // ShipVia
            $this->ShipVia->CurrentValue = $row['ShipVia'];
        }
        if (isset($row['Freight'])) { // Freight
            $this->Freight->CurrentValue = $row['Freight'];
        }
        if (isset($row['ShipName'])) { // ShipName
            $this->ShipName->CurrentValue = $row['ShipName'];
        }
        if (isset($row['ShipAddress'])) { // ShipAddress
            $this->ShipAddress->CurrentValue = $row['ShipAddress'];
        }
        if (isset($row['ShipCity'])) { // ShipCity
            $this->ShipCity->CurrentValue = $row['ShipCity'];
        }
        if (isset($row['ShipRegion'])) { // ShipRegion
            $this->ShipRegion->CurrentValue = $row['ShipRegion'];
        }
        if (isset($row['ShipPostalCode'])) { // ShipPostalCode
            $this->ShipPostalCode->CurrentValue = $row['ShipPostalCode'];
        }
        if (isset($row['ShipCountry'])) { // ShipCountry
            $this->ShipCountry->CurrentValue = $row['ShipCountry'];
        }
    }
