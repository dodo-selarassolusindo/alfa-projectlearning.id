 ...
 * Page class
 */
class SuppliersView extends Suppliers
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SuppliersView";
    ...
    // CSS class/style
    public $CurrentPageName = "suppliersview";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("SupplierID") ?? Route("SupplierID")) !== null) {
            $this->RecKey["SupplierID"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("SupplierID") ?? Route("SupplierID")) !== null) {
            ...
            $this->RecKey["SupplierID"] = $this->SupplierID->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->SupplierID->setQueryStringValue($keyValue);
            $this->RecKey["SupplierID"] = $this->SupplierID->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "supplierslist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("supplierslist"); // Return to list page
                        return;
                        ...
                        while ($this->fetch()) {
                            if (SameString($this->SupplierID->CurrentValue, $this->CurrentRow['SupplierID'])) {
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "supplierslist"; // No matching record, return to list
                    } else {
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fsuppliersview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . ArrayToJsonAttribute($this->RecKey) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
