...
class CarsList extends Cars
{
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcarssrch", $filters);
    ...
    {
        if ($this->isAddOrEdit() && ($this->RowType == RowType::ADD || $this->RowType == RowType::EDIT)) {
            return "row"; // Full row
        }
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_cars", "data-rowtype" => RowType::ADD]);
