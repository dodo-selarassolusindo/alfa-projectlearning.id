...
class DjiEdit extends Dji
{
    ...
    // CSS class/style
    public $CurrentPageName = "djiedit";
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("djilist"); // Return to list page
                        return;
    ...
    protected function getEditRow($rsold)
    {
        ...
        // Open
        $this->Open->setDbValueDef($rsnew, $this->Open->CurrentValue, $this->Open->ReadOnly);

        // High
        $this->High->setDbValueDef($rsnew, $this->High->CurrentValue, $this->High->ReadOnly);

        // Low
        $this->Low->setDbValueDef($rsnew, $this->Low->CurrentValue, $this->Low->ReadOnly);

        // Close
        $this->Close->setDbValueDef($rsnew, $this->Close->CurrentValue, $this->Close->ReadOnly);

        // Volume
        $this->Volume->setDbValueDef($rsnew, $this->Volume->CurrentValue, $this->Volume->ReadOnly);

        // Adj Close
        $this->AdjClose->setDbValueDef($rsnew, $this->AdjClose->CurrentValue, $this->AdjClose->ReadOnly);

        // Name
        $this->Name->setDbValueDef($rsnew, UnFormatDateTime($this->Name->CurrentValue, $this->Name->formatPattern()), $this->Name->ReadOnly);

        // Name2
        $this->Name2->setDbValueDef($rsnew, $this->Name2->CurrentValue, $this->Name2->ReadOnly);
        return $rsnew;
    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['Open'])) { // Open
            $this->Open->CurrentValue = $row['Open'];
        }
        if (isset($row['High'])) { // High
            $this->High->CurrentValue = $row['High'];
        }
        if (isset($row['Low'])) { // Low
            $this->Low->CurrentValue = $row['Low'];
        }
        if (isset($row['Close'])) { // Close
            $this->Close->CurrentValue = $row['Close'];
        }
        if (isset($row['Volume'])) { // Volume
            $this->Volume->CurrentValue = $row['Volume'];
        }
        if (isset($row['Adj Close'])) { // Adj Close
            $this->AdjClose->CurrentValue = $row['Adj Close'];
        }
        if (isset($row['Name'])) { // Name
            $this->Name->CurrentValue = $row['Name'];
        }
        if (isset($row['Name2'])) { // Name2
            $this->Name2->CurrentValue = $row['Name2'];
        }
    }
