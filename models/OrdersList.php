...
class OrdersList extends Orders
{
    ...
    public function __construct()
    {
        ...
        // Initialize URLs
        $this->AddUrl = "ordersadd?" . Config("TABLE_SHOW_DETAIL") . "=";
    ...
    public function run()
    {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "customers") {
            $masterTbl = Container("customers");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetchAssociative();
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("customerslist"); // Return to master page
                return;
            } else {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("forderssrch", $filters);
    ...
    public function advancedSearchWhere($default = false)
    {
        ...
        $this->buildSearchSql($where, $this->CustomerID, $default, false); // CustomerID
        ...
        $this->buildSearchSql($where, $this->RequiredDate, $default, false); // RequiredDate
        $this->buildSearchSql($where, $this->ShippedDate, $default, false); // ShippedDate
        $this->buildSearchSql($where, $this->ShipVia, $default, false); // ShipVia
        $this->buildSearchSql($where, $this->Freight, $default, false); // Freight
        $this->buildSearchSql($where, $this->ShipName, $default, false); // ShipName
        $this->buildSearchSql($where, $this->ShipAddress, $default, false); // ShipAddress
        $this->buildSearchSql($where, $this->ShipCity, $default, false); // ShipCity
        $this->buildSearchSql($where, $this->ShipRegion, $default, false); // ShipRegion
        $this->buildSearchSql($where, $this->ShipPostalCode, $default, false); // ShipPostalCode
        $this->buildSearchSql($where, $this->ShipCountry, $default, false); // ShipCountry
        ...
        if (!$default && $this->Command == "search") {
            ...
            $this->CustomerID->AdvancedSearch->save(); // CustomerID
            $this->EmployeeID->AdvancedSearch->save(); // EmployeeID
            $this->OrderDate->AdvancedSearch->save(); // OrderDate
            $this->RequiredDate->AdvancedSearch->save(); // RequiredDate
            $this->ShippedDate->AdvancedSearch->save(); // ShippedDate
            $this->ShipVia->AdvancedSearch->save(); // ShipVia
            $this->Freight->AdvancedSearch->save(); // Freight
            $this->ShipName->AdvancedSearch->save(); // ShipName
            $this->ShipAddress->AdvancedSearch->save(); // ShipAddress
            $this->ShipCity->AdvancedSearch->save(); // ShipCity
            $this->ShipRegion->AdvancedSearch->save(); // ShipRegion
            $this->ShipPostalCode->AdvancedSearch->save(); // ShipPostalCode
            $this->ShipCountry->AdvancedSearch->save(); // ShipCountry
    ...
    public function showFilterList()
    {
        ...
        // Field OrderID
        $filter = $this->queryBuilderWhere("OrderID");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->OrderID, false, false);
        }
        if ($filter != "") {
        ...
        // Field CustomerID
        $filter = $this->queryBuilderWhere("CustomerID");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->CustomerID, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->CustomerID->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field EmployeeID
        $filter = $this->queryBuilderWhere("EmployeeID");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->EmployeeID, false, false);
        }
        if ($filter != "") {
            $filterList .= "<div><span class=\"" . $captionClass . "\">" . $this->EmployeeID->caption() . "</span>" . $captionSuffix . $filter . "</div>";
        }

        // Field OrderDate
        $filter = $this->queryBuilderWhere("OrderDate");
        if (!$filter) {
            $this->buildSearchSql($filter, $this->OrderDate, false, false);
        }
        if ($filter != "") {
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->OrderID;
        $searchFlds[] = &$this->CustomerID;
        $searchFlds[] = &$this->EmployeeID;
        $searchFlds[] = &$this->OrderDate;
        $searchFlds[] = &$this->ShipVia;
        $searchFlds[] = &$this->Freight;
    ...
    protected function checkSearchParms()
    {
        ...
        if ($this->CustomerID->AdvancedSearch->issetSession()) {
            return true;
        }
        ...
        if ($this->RequiredDate->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShippedDate->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipVia->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->Freight->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipName->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipAddress->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipCity->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipRegion->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipPostalCode->AdvancedSearch->issetSession()) {
            return true;
        }
        if ($this->ShipCountry->AdvancedSearch->issetSession()) {
            return true;
        }
        return false;
    }
    ...
    protected function resetAdvancedSearchParms()
    {
        ...
        $this->CustomerID->AdvancedSearch->unsetSession();
        $this->EmployeeID->AdvancedSearch->unsetSession();
        $this->OrderDate->AdvancedSearch->unsetSession();
        $this->RequiredDate->AdvancedSearch->unsetSession();
        $this->ShippedDate->AdvancedSearch->unsetSession();
        $this->ShipVia->AdvancedSearch->unsetSession();
        $this->Freight->AdvancedSearch->unsetSession();
        $this->ShipName->AdvancedSearch->unsetSession();
        $this->ShipAddress->AdvancedSearch->unsetSession();
        $this->ShipCity->AdvancedSearch->unsetSession();
        $this->ShipRegion->AdvancedSearch->unsetSession();
        $this->ShipPostalCode->AdvancedSearch->unsetSession();
        $this->ShipCountry->AdvancedSearch->unsetSession();
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if ($this->Command != "json") {
            $defaultSort = $this->OrderID->Expression . " DESC"; // Set up default sort
    ...
    protected function setupListOptions()
    {
        ...
        $item = &$this->ListOptions->add("detail_order_details_extended");
        $item->CssClass = "text-nowrap";
    ...
    public function renderListOptions()
    {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"forderslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"forderslist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
        ...
        // "detail_orderdetails"
        $opt = $this->ListOptions["detail_orderdetails"];
        if ($Security->allowList(CurrentProjectID() . 'orderdetails') && $this->showOptionLink()) {
            ...
            if (!$this->ShowMultipleDetails) { // Skip loading record count if show multiple details
                ...
                $body .= "&nbsp;" . str_replace(["%c", "%s"], [Container("orderdetails")->Count, "blue"], $Language->phrase("DetailCount"));
            }
            ...
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=orderdetails");
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=orderdetails");
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=orderdetails");
        ...
        // "detail_order_details_extended"
        $opt = $this->ListOptions["detail_order_details_extended"];
        if ($Security->allowList(CurrentProjectID() . 'order details extended') && $this->showOptionLink()) {
            $body = $Language->phrase("DetailLink") . $Language->tablePhrase("order_details_extended", "TblCaption");
            if (!$this->ShowMultipleDetails) { // Skip loading record count if show multiple details
                ...
                $detailFilter = $detailTbl->applyUserIDFilters($detailFilter);
                $detailTbl->Count = $detailTbl->loadRecordCount($detailFilter);
                $body .= "&nbsp;" . str_replace(["%c", "%s"], [Container("order_details_extended")->Count, "blue"], $Language->phrase("DetailCount"));
            }
            ...
            if ($detailPage->DetailView && $Security->canView() && $this->showOptionLink("view") && $Security->allowView(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailViewLink", null);
                $url = $this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=order_details_extended");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailViewTblVar != "") {
                    $detailViewTblVar .= ",";
                }
            ...
            if ($detailPage->DetailEdit && $Security->canEdit() && $this->showOptionLink("edit") && $Security->allowEdit(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailEditLink", null);
                $url = $this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=order_details_extended");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailEditTblVar != "") {
                    $detailEditTblVar .= ",";
                }
            ...
            if ($detailPage->DetailAdd && $Security->canAdd() && $this->showOptionLink("add") && $Security->allowAdd(CurrentProjectID() . 'orders')) {
                $caption = $Language->phrase("MasterDetailCopyLink", null);
                $url = $this->getCopyUrl(Config("TABLE_SHOW_DETAIL") . "=order_details_extended");
                $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-copy\" data-action=\"add\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode($url) . "\">" . $caption . "</a></li>";
                if ($detailCopyTblVar != "") {
                    $detailCopyTblVar .= ",";
                }
    ...
    protected function setupOtherOptions()
    {
        ...
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=orderdetails");
        $detailPage = Container("OrderdetailsGrid");
        ...
        $item = &$option->add("detailadd_order_details_extended");
        $url = $this->getAddUrl(Config("TABLE_SHOW_DETAIL") . "=order_details_extended");
        $detailPage = Container("OrderDetailsExtendedGrid");
        $caption = $Language->phrase("Add") . "&nbsp;" . $this->tableCaption() . "/" . $detailPage->tableCaption();
        $item->Body = "<a class=\"ew-detail-add-group ew-detail-add\" title=\"" . HtmlTitle($caption) . "\" data-caption=\"" . HtmlTitle($caption) . "\" href=\"" . HtmlEncode(GetUrl($url)) . "\">" . $caption . "</a>";
        $item->Visible = ($detailPage->DetailAdd && $Security->allowAdd(CurrentProjectID() . 'orders') && $Security->canAdd());
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "OrderID");
            $this->createColumnOption($option, "CustomerID");
            $this->createColumnOption($option, "EmployeeID");
            $this->createColumnOption($option, "OrderDate");
        }
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="forderslist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_orders", "data-rowtype" => RowType::ADD]);
    ...
    protected function loadSearchValues()
    {
        ...
        if ($this->CustomerID->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->CustomerID->AdvancedSearch->SearchValue != "" || $this->CustomerID->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->OrderDate->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->OrderDate->AdvancedSearch->SearchValue != "" || $this->OrderDate->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->RequiredDate->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->RequiredDate->AdvancedSearch->SearchValue != "" || $this->RequiredDate->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShippedDate->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShippedDate->AdvancedSearch->SearchValue != "" || $this->ShippedDate->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipVia->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipVia->AdvancedSearch->SearchValue != "" || $this->ShipVia->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->Freight->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->Freight->AdvancedSearch->SearchValue != "" || $this->Freight->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipName->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipName->AdvancedSearch->SearchValue != "" || $this->ShipName->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipAddress->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipAddress->AdvancedSearch->SearchValue != "" || $this->ShipAddress->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipCity->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipCity->AdvancedSearch->SearchValue != "" || $this->ShipCity->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipRegion->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipRegion->AdvancedSearch->SearchValue != "" || $this->ShipRegion->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipPostalCode->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipPostalCode->AdvancedSearch->SearchValue != "" || $this->ShipPostalCode->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        ...
        if ($this->ShipCountry->AdvancedSearch->get()) {
            $hasValue = true;
            if (($this->ShipCountry->AdvancedSearch->SearchValue != "" || $this->ShipCountry->AdvancedSearch->SearchValue2 != "") && $this->Command == "") {
                $this->Command = "search";
            }
        }
        return $hasValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->OrderID->EditValue = $this->OrderID->AdvancedSearch->SearchValue;
            ...
            if ($this->CustomerID->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->CustomerID->Lookup->getTable()->Fields["CustomerID"]->searchExpression(), "=", $this->CustomerID->AdvancedSearch->SearchValue, $this->CustomerID->Lookup->getTable()->Fields["CustomerID"]->searchDataType(), "");
                }
    ...
    protected function setupSearchOptions()
    {
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-table=\"orders\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" data-ew-action=\"modal\" data-url=\"ordersquery\" data-btn=\"SearchBtn\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" href=\"ordersquery\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        }
        $item->Visible = true;
    ...
    */
    public function exportData($doc)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->DbMasterFilter != "" && $this->getCurrentMasterTable() == "customers") {
            $customers = new CustomersList();
            $rsmaster = $customers->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->setTable($customers);
                    $customers->exportDocument($doc, $rsmaster);
                    $doc->exportEmptyRow();
                    $doc->setTable($this);
                }
            }
