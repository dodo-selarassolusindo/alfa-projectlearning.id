...
class QuarterlyOrdersByProductCrosstab extends QuarterlyOrdersByProduct
{
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Quarterly_Orders_By_Product"]) || $GLOBALS["Quarterly_Orders_By_Product"]::class == PROJECT_NAMESPACE . "Quarterly_Orders_By_Product") {
    ...
    protected function setupSearchOptions()
    {
        ...
        if ($this->ModalSearch && !IsMobile()) {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-table=\"Quarterly_Orders_By_Product\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" data-ew-action=\"modal\" data-url=\"\" data-btn=\"SearchBtn\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        } else {
            $item->Body = "<a class=\"btn btn-default ew-query-builder\" title=\"" . $Language->phrase("QueryBuilder", true) . "\" data-caption=\"" . $Language->phrase("QueryBuilder", true) . "\" href=\"\">" . $Language->phrase("QueryBuilder", false) . "</a>";
        }
        $item->Visible = true;
    ...
    public function getFilterList()
    {
        ...
        // Field OrderDate
        $wrk = "";
        $wrk = $this->OrderDate->AdvancedSearch->SearchValue;
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
        ...
        if ($parm >= 0 && $sql == "") {
            switch ($fld->Param) {
                case "CategoryName":
                    ...
                    break;
                case "OrderDate":
                    if (!EmptyValue($fld->AdvancedSearch->SearchValue)) {
                        $sql = "YEAR(%s)";
                        $sql = str_replace("%s", "@" . $target, $sql) . " = " . QuotedValue($fld->AdvancedSearch->SearchValue, DataType::NUMBER, $this->Dbid);
                    }
                    break;
            }
        }
