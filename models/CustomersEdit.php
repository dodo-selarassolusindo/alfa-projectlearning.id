...
class CustomersEdit extends Customers
{
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (CustomerID)
        if ($this->CustomerID->CurrentValue != "") {
            $filterChk = "(`CustomerID` = '" . AdjustSql($this->CustomerID->CurrentValue, $this->Dbid) . "')";
        ...
        if ($updateRow) {
            ...
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("OrdersGrid");
            if (in_array("orders", $detailTblVar) && $detailPage->DetailEdit && $editRow) {
