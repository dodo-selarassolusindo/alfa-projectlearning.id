 ...
 * Page class
 */
class SalesByCustomerSummary extends SalesByCustomer
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SalesByCustomerSummary";
    ...
    public $CurrentPageName = "salesbycustomer";
    ...
    public function __construct()
    {
        ...
        // Table object (Sales_By_Customer)
        if (!isset($GLOBALS["Sales_By_Customer"]) || $GLOBALS["Sales_By_Customer"]::class == PROJECT_NAMESPACE . "Sales_By_Customer") {
            $GLOBALS["Sales_By_Customer"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Sales By Customer');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) {
            ...
            $rs = $sql->executeQuery();
            $records = $rs?->fetchAll() ?? [];
            $this->Quantity->getAvg($records, false);
            ...
            if ($rsagg) {
                ...
                $this->Quantity->AvgValue = $rsagg["avg_quantity"];
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->Quantity->getAvg($this->DetailRecords, false);
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            // Quantity
            $this->Quantity->AvgViewValue = $this->Quantity->AvgValue;
            $this->Quantity->AvgViewValue = FormatNumber($this->Quantity->AvgViewValue, $this->Quantity->formatPattern());
            $this->Quantity->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            // Quantity
            $currentValue = $this->Quantity->AvgValue;
            $viewValue = &$this->Quantity->AvgViewValue;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fSales_By_Customersrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fSales_By_Customersrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
