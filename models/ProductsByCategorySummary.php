 ...
 * Page class
 */
class ProductsByCategorySummary extends ProductsByCategory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "ProductsByCategorySummary";
    ...
    public $CurrentPageName = "productsbycategory";
    ...
    public function __construct()
    {
        ...
        // Table object (Products_By_Category)
        if (!isset($GLOBALS["Products_By_Category"]) || $GLOBALS["Products_By_Category"]::class == PROJECT_NAMESPACE . "Products_By_Category") {
            $GLOBALS["Products_By_Category"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Products By Category');
        }
    ...
    public function loadRowValues($record)
    {
        ...
        $data["Discontinued"] = $record['Discontinued'];
        $this->Rows[] = $data;
        ...
        $this->Discontinued->setDbValue($record['Discontinued']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            $this->Discontinued->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            // Discontinued
            $currentValue = $this->Discontinued->CurrentValue;
            $viewValue = &$this->Discontinued->ViewValue;
            $viewAttrs = &$this->Discontinued->ViewAttrs;
            $cellAttrs = &$this->Discontinued->CellAttrs;
            $hrefValue = &$this->Discontinued->HrefValue;
            $linkAttrs = &$this->Discontinued->LinkAttrs;
            $this->cellRendered($this->Discontinued, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fProducts_By_Categorysrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fProducts_By_Categorysrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
