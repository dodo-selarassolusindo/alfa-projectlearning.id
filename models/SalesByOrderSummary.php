 ...
 * Page class
 */
class SalesByOrderSummary extends SalesByOrder
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SalesByOrderSummary";
    ...
    public $CurrentPageName = "salesbyorder";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Sales_By_Order';
        $this->TableName = 'Sales By Order';
        ...
        // Table object (Sales_By_Order)
        if (!isset($GLOBALS["Sales_By_Order"]) || $GLOBALS["Sales_By_Order"]::class == PROJECT_NAMESPACE . "Sales_By_Order") {
            $GLOBALS["Sales_By_Order"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Sales By Order');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->OrderID->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->OrderID->setGroupValue("");
        }
    }
    ...
    public function loadRowValues($record)
    {
        ...
        $this->Rows[] = $data;
        $this->CompanyName->setDbValue($record['CompanyName']);
        $this->OrderID->setDbValue(GroupValue($this->OrderID, $record['OrderID']));
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) {
            ...
            $rs = $sql->executeQuery();
            $records = $rs?->fetchAll() ?? [];
            $this->ExtendedPrice->getSum($records, false);
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->ExtendedPrice->getSum($this->DetailRecords, false);
            }
        }
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            $arwrk = [];
            $arwrk["lf"] = $this->CompanyName->CurrentValue;
            $arwrk["df"] = $this->CompanyName->CurrentValue;
            $arwrk = $this->CompanyName->Lookup->renderViewRow($arwrk, $this);
            $dispVal = $this->CompanyName->displayValue($arwrk);
            if ($dispVal != "") {
                $this->CompanyName->EditValue = $dispVal;
            }
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->OrderID->groupValue(); // Set up group attribute
            }
            if ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->CompanyName->groupValue(); // Set up group attribute 2
            }

            // OrderID
            $this->OrderID->GroupViewValue = $this->OrderID->groupValue();
            $this->OrderID->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            ...
            $dispVal = $this->CompanyName->displayValue($arwrk);
            if ($dispVal != "") {
                $this->CompanyName->GroupViewValue = $dispVal;
            }
            $this->CompanyName->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            ...
            $this->ExtendedPrice->SumViewValue = FormatNumber($this->ExtendedPrice->SumViewValue, $this->ExtendedPrice->formatPattern());
            $this->ExtendedPrice->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            // OrderID
            $this->OrderID->GroupViewValue = $this->OrderID->groupValue();
            $this->OrderID->CellCssClass = "ew-rpt-grp-field-1";
            $this->OrderID->GroupViewValue = DisplayGroupValue($this->OrderID, $this->OrderID->GroupViewValue);
            ...
            $this->CompanyName->CellCssClass = "ew-rpt-grp-field-2";
            $this->CompanyName->GroupViewValue = DisplayGroupValue($this->CompanyName, $this->CompanyName->GroupViewValue);
            ...
            $this->UnitPrice->ViewValue = FormatNumber($this->UnitPrice->ViewValue, $this->UnitPrice->formatPattern());
            ...
            $this->ExtendedPrice->ViewValue = FormatNumber($this->ExtendedPrice->ViewValue, $this->ExtendedPrice->formatPattern());
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fSales_By_Ordersrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->CompanyName->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fSales_By_Ordersrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fSales_By_Ordersrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd") == "reset") {
            ...
            if ($this->CompanyName->DataType == DataType::DATE) { // Format default date format
                $this->CompanyName->AdvancedSearch->SearchValue = FormatDateTime($this->CompanyName->AdvancedSearch->SearchValue, 0);
            }
            }
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->CompanyName->AdvancedSearch->issetSession()) { // Field CompanyName
        ...
        $this->buildExtendedFilter($this->CompanyName, $filter, false, true); // Field CompanyName
    ...
    protected function validateForm()
    {
        ...
        if (!Config("SERVER_VALIDATE")) {
            return true;
                ...
                if (!CheckNumber($this->ExtendedPrice->FormValue)) {
                    $this->ExtendedPrice->addErrorMessage($this->ExtendedPrice->getErrorMessage(false));
                }
    ...
    {
        // Field CompanyName
        $this->CompanyName->AdvancedSearch->loadDefault();
    }
    ...
    public function showFilterList()
    {
        ...
        // Field CompanyName
        $extWrk = "";
        $this->buildExtendedFilter($this->CompanyName, $extWrk);
        $filter = "";
        if ($extWrk != "") {
    ...
    public function getFilterList()
    {
        ...
        // Field CompanyName
        $wrk = "";
        if ($this->CompanyName->AdvancedSearch->SearchValue != "" || $this->CompanyName->AdvancedSearch->SearchValue2 != "") {
            $wrk = "\"x_CompanyName\":\"" . JsEncode($this->CompanyName->AdvancedSearch->SearchValue) . "\"," .
                "\"z_CompanyName\":\"" . JsEncode($this->CompanyName->AdvancedSearch->SearchOperator) . "\"," .
                "\"v_CompanyName\":\"" . JsEncode($this->CompanyName->AdvancedSearch->SearchCondition) . "\"," .
                "\"y_CompanyName\":\"" . JsEncode($this->CompanyName->AdvancedSearch->SearchValue2) . "\"," .
                "\"w_CompanyName\":\"" . JsEncode($this->CompanyName->AdvancedSearch->SearchOperator2) . "\"";
        }
        if ($wrk != "") {
            if ($filterList != "") {
                $filterList .= ",";
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field CompanyName
        if (!$this->CompanyName->AdvancedSearch->get($filter)) {
            $this->CompanyName->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->CompanyName->AdvancedSearch->save();
        return true;
    }
    ...
    protected function getDrillDownFilter()
    {
        ...
        if ($opt == "1" || $opt == "2") {
            $mastertable = Param("s", ""); // Get source table
                $sql = Param("CompanyName", "");
                $sql = Decrypt($sql);
                $sql = str_replace("@CompanyName", "`CompanyName`", $sql);
                if ($sql != "") {
                    if ($filter != "") {
                        $filter .= " AND ";
                    }
                    $filter .= $sql;
                    if ($sql != "1=1") {
                        $filterList .= "<div><span class=\"ew-filter-caption\">" . $this->CompanyName->caption() . "</span><span class=\"ew-filter-value\">$sql</span></div>";
                    }
                }
                $sql = Param("OrderID", "");
                $sql = Decrypt($sql);
                $sql = str_replace("@OrderID", "`OrderID`", $sql);
                if ($sql != "") {
                    if ($filter != "") {
                        $filter .= " AND ";
                    }
                    $filter .= $sql;
                    if ($sql != "1=1") {
                        $filterList .= "<div><span class=\"ew-filter-caption\">" . $this->OrderID->caption() . "</span><span class=\"ew-filter-value\">$sql</span></div>";
                    }
                }
        }
        if ($opt == "1" || $opt == "2") {
        ...
        if (!EmptyValue($filter)) {
            if (EmptyValue($filterList) && !EmptyValue($Language->phrase("DrillDownAllRecords"))) {
                $filterList = "<div><span class=\"ew-filter-value\">" . $Language->phrase("DrillDownAllRecords") . "</span></div>";
            }
            if (!EmptyValue($filterList)) {
    ...
    public function showDrillDownList()
    {
        ...
        if ($this->DrillDownList != "") {
            $message = "<div id=\"ew-drilldown-list\"" . $divclass . "><div class=\"callout callout-info d-table\">" . $this->DrillDownList . "</div></div>";
            $this->messageShowing($message, "");
            Write($message);
        } else { // Output empty tag
            Write("<div id=\"ew-filter-list\"></div>");
