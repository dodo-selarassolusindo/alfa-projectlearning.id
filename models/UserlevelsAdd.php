...
class UserlevelsAdd extends Userlevels
{
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (strval($this->userlevelid->EditValue) != "" && is_numeric($this->userlevelid->EditValue)) {
                $this->userlevelid->EditValue = $this->userlevelid->EditValue;
            }
    ...
    protected function addRow($rsold = null)
    {
        ...
        } elseif ((int)$this->userlevelid->CurrentValue < AdvancedSecurity::ANONYMOUS_USER_LEVEL_ID) {
            $this->setFailureMessage($Language->phrase("UserLevelIDIncorrect"));
        } elseif ((int)$this->userlevelid->CurrentValue == AdvancedSecurity::DEFAULT_USER_LEVEL_ID && !SameText($this->userlevelname->CurrentValue, "Default")) {
            $this->setFailureMessage($Language->phrase("UserLevelDefaultName"));
        } elseif ((int)$this->userlevelid->CurrentValue == AdvancedSecurity::ADMIN_USER_LEVEL_ID && !SameText($this->userlevelname->CurrentValue, "Administrator")) {
            $this->setFailureMessage($Language->phrase("UserLevelAdministratorName"));
        } elseif ((int)$this->userlevelid->CurrentValue == AdvancedSecurity::ANONYMOUS_USER_LEVEL_ID && !SameText($this->userlevelname->CurrentValue, "Anonymous")) {
            $this->setFailureMessage($Language->phrase("UserLevelAnonymousName"));
        } elseif ((int)$this->userlevelid->CurrentValue > AdvancedSecurity::DEFAULT_USER_LEVEL_ID && in_array(strtolower(trim($this->userlevelname->CurrentValue)), ["anonymous", "administrator", "default"])) {
    ...
    protected function getAddRow()
    {
        ...
        // userlevelid
        $this->userlevelid->setDbValueDef($rsnew, $this->userlevelid->CurrentValue, false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        if (isset($row['userlevelid'])) { // userlevelid
            $this->userlevelid->setFormValue($row['userlevelid']);
        }
        if (isset($row['userlevelname'])) { // userlevelname
            $this->userlevelname->setFormValue($row['userlevelname']);
        }
    }
