...
class Orders extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->CustomerID->Lookup = new Lookup($this->CustomerID, 'customers', false, 'CustomerID', ["CompanyName","CustomerID","",""], '', '', [], [], [], [], [], [], false, '', '<div>{{:df}} <em>({{:df2}})</em></div>', "CONCAT(COALESCE(`CompanyName`, ''),'" . ValueSeparator(1, $this->CustomerID) . "',COALESCE(`CustomerID`,''))");
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($cascadeUpdate) {
            $rswrk = Container("orderdetails")->loadRs("`OrderID` = " . QuotedValue($rsold['OrderID'], DataType::NUMBER, 'DB'))->fetchAllAssociative();
    ...
    public function delete(&$rs, $where = "", $curfilter = false)
    {
        ...
        $dtlrows = Container("orderdetails")->loadRs("`OrderID` = " . QuotedValue($rs['OrderID'], DataType::NUMBER, "DB"))->fetchAllAssociative();
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ordersview" => $Language->phrase("View"),
            "ordersedit" => $Language->phrase("Edit"),
            "ordersadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "OrdersView",
            Config("API_ADD_ACTION") => "OrdersAdd",
            Config("API_EDIT_ACTION") => "OrdersEdit",
            Config("API_DELETE_ACTION") => "OrdersDelete",
            Config("API_LIST_ACTION") => "OrdersList",
            default => ""
        };
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ordersdelete", $parm);
        }
    }
