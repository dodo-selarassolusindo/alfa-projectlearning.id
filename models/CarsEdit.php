...
class CarsEdit extends Cars
{
    ...
    // CSS class/style
    public $CurrentPageName = "carsedit";
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("carslist"); // Return to list page
                        return;
            ...
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    if (!$loaded) {
                        ...
                        $this->HashValue = $this->getRowHash($this->CurrentRow); // Get hash value for record
                    }
    ...
    protected function getEditRow($rsold)
    {
        ...
        // Transmission Speeds
        $this->TransmissionSpeeds->setDbValueDef($rsnew, $this->TransmissionSpeeds->CurrentValue, $this->TransmissionSpeeds->ReadOnly);
        ...
        $this->TransmissAutomatic->setDbValueDef($rsnew, $tmpBool, $this->TransmissAutomatic->ReadOnly);

        // MPG City
        $this->MPGCity->setDbValueDef($rsnew, $this->MPGCity->CurrentValue, $this->MPGCity->ReadOnly);

        // MPG Highway
        $this->MPGHighway->setDbValueDef($rsnew, $this->MPGHighway->CurrentValue, $this->MPGHighway->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['Cylinders'])) { // Cylinders
            $this->Cylinders->CurrentValue = $row['Cylinders'];
        }
        if (isset($row['Transmission Speeds'])) { // Transmission Speeds
            $this->TransmissionSpeeds->CurrentValue = $row['Transmission Speeds'];
        }
        if (isset($row['TransmissAutomatic'])) { // TransmissAutomatic
            $this->TransmissAutomatic->CurrentValue = $row['TransmissAutomatic'];
        }
        if (isset($row['MPG City'])) { // MPG City
            $this->MPGCity->CurrentValue = $row['MPG City'];
        }
        if (isset($row['MPG Highway'])) { // MPG Highway
            $this->MPGHighway->CurrentValue = $row['MPG Highway'];
        }
        ...
        if (isset($row['Doors'])) { // Doors
            $this->Doors->CurrentValue = $row['Doors'];
        }
        if (isset($row['Torque'])) { // Torque
            $this->Torque->CurrentValue = $row['Torque'];
        }
    }
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['Transmission Speeds']); // Transmission Speeds
        $hash .= GetFieldHash($row['TransmissAutomatic']); // TransmissAutomatic
        $hash .= GetFieldHash($row['MPG City']); // MPG City
        $hash .= GetFieldHash($row['MPG Highway']); // MPG Highway
