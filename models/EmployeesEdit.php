...
class EmployeesEdit extends Employees
{
                ...
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        if ($this->EmployeeID->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->EmployeeID->CurrentValue, $this->CurrentRow['EmployeeID'])) {
            ...
            if (!$Security->isAdmin() && $Security->isLoggedIn()) { // Non system admin
                if (SameString($this->EmployeeID->CurrentValue, CurrentUserID())) {
                    ...
                    if (trim(strval($this->ReportsTo->CurrentValue)) == "") {
                        ...
                        $filterWrk = SearchFilter($this->ReportsTo->Lookup->getTable()->Fields["EmployeeID"]->searchExpression(), "=", $this->ReportsTo->CurrentValue, $this->ReportsTo->Lookup->getTable()->Fields["EmployeeID"]->searchDataType(), "");
                    }
    ...
    protected function editRow()
    {
        ...
        if ($updateRow) {
            ...
            if ($editRow) {
                if ($this->Photo->Visible && !$this->Photo->Upload->KeepFile) {
                    if (!SaveUploadFiles($this->Photo, $rsnew['Photo'], true)) {
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['Extension'])) { // Extension
            $this->Extension->CurrentValue = $row['Extension'];
        }
