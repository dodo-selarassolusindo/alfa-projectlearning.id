...
class Employees extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->TitleOfCourtesy->Lookup = new Lookup($this->TitleOfCourtesy, 'employees', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
        ...
        $this->ReportsTo->Lookup = new Lookup($this->ReportsTo, 'employees', false, 'EmployeeID', ["LastName","FirstName","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`LastName`, ''),'" . ValueSeparator(1, $this->ReportsTo) . "',COALESCE(`FirstName`,''))");
    ...
    public function renderListRow()
    {
        ...
        if ($curVal != "") {
            ...
            if ($this->ReportsTo->ViewValue === null) { // Lookup from database
                $filterWrk = SearchFilter($this->ReportsTo->Lookup->getTable()->Fields["EmployeeID"]->searchExpression(), "=", $curVal, $this->ReportsTo->Lookup->getTable()->Fields["EmployeeID"]->searchDataType(), "");
    ...
    public function prepareRegisterEmail($user, $langId = "")
    {
        ...
        $fields = [
            ...
            'LastName' => (object)[ "caption" => $this->LastName->caption(), "value" => $user->get('LastName') ],
            'FirstName' => (object)[ "caption" => $this->FirstName->caption(), "value" => $user->get('FirstName') ],
            'Email' => (object)[ "caption" => $this->_Email->caption(), "value" => $user->get('Email') ],
        ];
        $email = new Email();
        $data = [
