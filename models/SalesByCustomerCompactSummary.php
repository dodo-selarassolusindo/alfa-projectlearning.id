 ...
 * Page class
 */
class SalesByCustomerCompactSummary extends SalesByCustomerCompact
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SalesByCustomerCompactSummary";
    ...
    public $CurrentPageName = "salesbycustomercompact";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Sales_By_Customer_Compact';
        $this->TableName = 'Sales By Customer (Compact)';
        ...
        // Table object (Sales_By_Customer_Compact)
        if (!isset($GLOBALS["Sales_By_Customer_Compact"]) || $GLOBALS["Sales_By_Customer_Compact"]::class == PROJECT_NAMESPACE . "Sales_By_Customer_Compact") {
            $GLOBALS["Sales_By_Customer_Compact"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Sales By Customer (Compact)');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            $this->OrderID->CellCssClass = "ew-rpt-grp-field-%g";
            ...
            $this->ExtendedPrice->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : (($this->RowGroupLevel < 2) ? "ew-rpt-grp-summary-" . $this->RowGroupLevel : (($this->LastGroupCount % 2 != 1) ? "ew-table-alt-row" : ""));
            ...
            if (!$this->isExport() && AllowList(PROJECT_ID . $this->ExtendedPrice->DrillDownTable)) {
                $drillurl = $this->ExtendedPrice->DrillDownUrl;
                $drillurl = str_replace("=f0", "=" . Encrypt($this->getDrillDownSql($this->CompanyName, "CompanyName", $this->RowTotalType)), $drillurl);
                $parm = ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 2) ? 0 : -1;
                $drillurl = str_replace("=f1", "=" . Encrypt($this->getDrillDownSql($this->OrderID, "OrderID", $this->RowTotalType, $parm)), $drillurl);
                $this->ExtendedPrice->LinkAttrs["data-title"] = JsEncodeAttribute($GLOBALS["Language"]->phrase("ClickToDrillDown"));
                $this->ExtendedPrice->LinkAttrs["class"] = "ew-drill-link ew-tippy";
                $this->ExtendedPrice->LinkAttrs->merge(DrillDownAttributes($drillurl, 'Sales_By_Customer_Compact_ExtendedPrice', $GLOBALS["Language"]->tablePhrase('Sales_By_Order', 'TblCaption'), $this->UseDrillDownPanel));
                if ($this->ExtendedPrice->HrefValue == "") {
                    $this->ExtendedPrice->HrefValue = "#";
                }
            }
        } else {
            ...
            if (!$this->isExport() && AllowList(PROJECT_ID . $this->ExtendedPrice->DrillDownTable)) {
                $drillurl = $this->ExtendedPrice->DrillDownUrl;
                $drillurl = str_replace("=f0", "=" . Encrypt($this->getDrillDownSql($this->CompanyName, "CompanyName", RowSummary::DETAIL)), $drillurl);
                $drillurl = str_replace("=f1", "=" . Encrypt($this->getDrillDownSql($this->OrderID, "OrderID", RowSummary::DETAIL)), $drillurl);
                $this->ExtendedPrice->LinkAttrs["data-title"] = JsEncodeAttribute($GLOBALS["Language"]->phrase("ClickToDrillDown"));
                $this->ExtendedPrice->LinkAttrs["class"] = "ew-drill-link ew-tippy";
                $this->ExtendedPrice->LinkAttrs->merge(DrillDownAttributes($drillurl, 'Sales_By_Customer_Compact_ExtendedPrice', $GLOBALS["Language"]->tablePhrase('Sales_By_Order', 'TblCaption'), $this->UseDrillDownPanel));
                if ($this->ExtendedPrice->HrefValue == "") {
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fSales_By_Customer_Compactsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fSales_By_Customer_Compactsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
     ...
     * @param RowSummary $rowtype Row type
     * 0 = detail
     * 1 = group
     * 2 = page
     * 3 = grand
    ...
    public function getDrillDownSql($fld, $target, $rowtype, $parm = 0)
    {
        ...
        if ($fld->Param == "CompanyName") { // First grouping field
            if ($rowtype == RowSummary::GRAND) { // Grand total
                $sql = $fld->CurrentFilter;
                if ($sql == "") {
                    $sql = "1=1"; // Show all records
                }
            } elseif ($rowtype == RowSummary::PAGE && $this->PageFirstGroupFilter != "") { // Page total
        ...
        if ($parm >= 0 && $sql == "") {
            switch ($fld->Param) {
                case "CompanyName":
                    if ($fld->GroupSql != "") {
                        $sql = str_replace("%s", "@" . $target, $fld->GroupSql) . " = " . QuotedValue($fld->CurrentValue, DataType::STRING, $this->Dbid);
        ...
        // Detail field
        if ($sql == "" && $rowtype == RowSummary::DETAIL) {
