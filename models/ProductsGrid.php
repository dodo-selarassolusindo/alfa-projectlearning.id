...
class ProductsGrid extends Products
{
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->UnitsInStock->DefaultValue = $this->UnitsInStock->getDefault(); // PHP
        $this->UnitsInStock->OldValue = $this->UnitsInStock->DefaultValue;
        $this->UnitsOnOrder->DefaultValue = $this->UnitsOnOrder->getDefault(); // PHP
        $this->UnitsOnOrder->OldValue = $this->UnitsOnOrder->DefaultValue;
        $this->ReorderLevel->DefaultValue = $this->ReorderLevel->getDefault(); // PHP
        $this->ReorderLevel->OldValue = $this->ReorderLevel->DefaultValue;
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($this->SupplierID->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->SupplierID->Lookup->getTable()->Fields["SupplierID"]->searchExpression(), "=", $this->SupplierID->CurrentValue, $this->SupplierID->Lookup->getTable()->Fields["SupplierID"]->searchDataType(), "");
                }
            ...
            if ($this->CategoryID->getSessionValue() != "") {
                ...
                if ($this->CategoryID->ViewValue !== null) { // Load from cache
                    ...
                    if ($curVal == "") {
                        ...
                        $filterWrk = SearchFilter($this->CategoryID->Lookup->getTable()->Fields["CategoryID"]->searchExpression(), "=", $this->CategoryID->CurrentValue, $this->CategoryID->Lookup->getTable()->Fields["CategoryID"]->searchDataType(), "");
                    }
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['SupplierID'])) { // SupplierID
            $this->SupplierID->CurrentValue = $row['SupplierID'];
        }
        if (isset($row['CategoryID'])) { // CategoryID
            $this->CategoryID->CurrentValue = $row['CategoryID'];
        }
        if (isset($row['QuantityPerUnit'])) { // QuantityPerUnit
            $this->QuantityPerUnit->CurrentValue = $row['QuantityPerUnit'];
        }
        if (isset($row['UnitPrice'])) { // UnitPrice
            $this->UnitPrice->CurrentValue = $row['UnitPrice'];
        }
        if (isset($row['UnitsInStock'])) { // UnitsInStock
            $this->UnitsInStock->CurrentValue = $row['UnitsInStock'];
        }
        if (isset($row['UnitsOnOrder'])) { // UnitsOnOrder
            $this->UnitsOnOrder->CurrentValue = $row['UnitsOnOrder'];
        }
        if (isset($row['ReorderLevel'])) { // ReorderLevel
            $this->ReorderLevel->CurrentValue = $row['ReorderLevel'];
        }
        if (isset($row['Discontinued'])) { // Discontinued
            $this->Discontinued->CurrentValue = $row['Discontinued'];
        }
        if (isset($row['EAN13'])) { // EAN13
            $this->EAN13->CurrentValue = $row['EAN13'];
        }
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['SupplierID'])) { // SupplierID
            $this->SupplierID->setFormValue($row['SupplierID']);
        }
        if (isset($row['CategoryID'])) { // CategoryID
            $this->CategoryID->setFormValue($row['CategoryID']);
        }
        if (isset($row['QuantityPerUnit'])) { // QuantityPerUnit
            $this->QuantityPerUnit->setFormValue($row['QuantityPerUnit']);
        }
        if (isset($row['UnitPrice'])) { // UnitPrice
            $this->UnitPrice->setFormValue($row['UnitPrice']);
        }
        if (isset($row['UnitsInStock'])) { // UnitsInStock
            $this->UnitsInStock->setFormValue($row['UnitsInStock']);
        }
        if (isset($row['UnitsOnOrder'])) { // UnitsOnOrder
            $this->UnitsOnOrder->setFormValue($row['UnitsOnOrder']);
        }
        if (isset($row['ReorderLevel'])) { // ReorderLevel
            $this->ReorderLevel->setFormValue($row['ReorderLevel']);
        }
        if (isset($row['Discontinued'])) { // Discontinued
            $this->Discontinued->setFormValue($row['Discontinued']);
        }
        if (isset($row['EAN13'])) { // EAN13
            $this->EAN13->setFormValue($row['EAN13']);
        }
    }
