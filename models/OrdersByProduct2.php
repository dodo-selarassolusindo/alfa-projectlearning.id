...
/**
 * Table class for orders_by_product2
 */
class OrdersByProduct2 extends ReportTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // OrderByProducts
        $this->OrderByProducts = new DbChart($this, 'OrderByProducts', 'OrderByProducts', 'ProductName', 'Amount', 1001, '', 0, 'SUM', 550, 440);
        $this->OrderByProducts->Position = 4;
        $this->OrderByProducts->PageBreakType = "before";
        $this->OrderByProducts->YAxisFormat = ["Currency"];
        $this->OrderByProducts->YFieldFormat = ["Currency"];
        $this->OrderByProducts->SortType = 0;
        $this->OrderByProducts->SortSequence = "";
        $this->OrderByProducts->SqlSelect = $this->getQueryBuilder()->select("`ProductName`", "''", "SUM(`Amount`)");
        $this->OrderByProducts->SqlGroupBy = "`ProductName`";
        $this->OrderByProducts->SqlOrderBy = "";
        $this->OrderByProducts->SeriesDateType = "";
        $this->OrderByProducts->ID = "orders_by_product2_OrderByProducts"; // Chart ID
        $this->OrderByProducts->setParameters([
            ["type", "1001"],
            ["seriestype", "0"]
        ]); // Chart type / Chart series type
        $this->OrderByProducts->setParameter("bgcolor", "FCFCFC"); // Background color
        $this->OrderByProducts->setParameters([
            ["caption", $this->OrderByProducts->caption()],
            ["xaxisname", $this->OrderByProducts->xAxisName()]
        ]); // Chart caption / X axis name
        $this->OrderByProducts->setParameter("yaxisname", $this->OrderByProducts->yAxisName()); // Y axis name
        $this->OrderByProducts->setParameters([
            ["shownames", "1"],
            ["showvalues", "1"],
            ["showhovercap", "1"]
        ]); // Show names / Show values / Show hover
        $this->OrderByProducts->setParameter("alpha", DbChart::getDefaultAlpha()); // Chart alpha (datasets background color)
        $this->OrderByProducts->setParameters([["options.plugins.filler.propagate",false],["options.animation.animateRotate",false],["options.animation.animateScale",false],["dataset.showLine",false],["dataset.spanGaps",false],["dataset.steppedLine",false],["scale.gridLines.offsetGridLines",false],["annotation1.show",true],["annotation1.startValue",100000],["annotation1.endValue",100000],["annotation1.displayValue","Popular"],["annotation1.secondaryYAxis",false],["annotation2.show",false],["annotation2.secondaryYAxis",false],["annotation3.show",false],["annotation3.secondaryYAxis",false],["annotation4.show",false],["annotation4.secondaryYAxis",false],["options.plugins.legend.display",false],["options.plugins.legend.fullWidth",false],["options.plugins.legend.reverse",false],["options.plugins.legend.labels.usePointStyle",false],["options.plugins.title.display",false],["options.plugins.tooltip.enabled",true],["options.plugins.tooltip.mode","point"],["options.plugins.tooltip.intersect",false],["options.plugins.tooltip.displayColors",false],["options.plugins.legend.rtl",false],["options.plugins.tooltip.rtl",false],["options.scales.r.angleLines.display",false],["options.plugins.stacked100.enable",false],["dataset.circular",false],["scale.offset",false],["options.plugins.datalabels.clamp",false],["options.plugins.datalabels.clip",false],["options.plugins.datalabels.display",false]]);
        $this->OrderByProducts->Trends[] = [
            "value" => 100000,
            "endValue" => 100000,
            "borderColor" => "",
            "label" => "Popular",
            "borderWidth" => 1,
            "alpha" => DbChart::getDefaultAlpha(),
            "parentYAxis" => ""
        ];
        $this->Charts[$this->OrderByProducts->ID] = &$this->OrderByProducts;
    ...
    {
        return "ordersbyproduct2";
    }

    // API page name
    public function getApiPageName($action)
    {
        return "OrdersByProduct2Summary";
    }
