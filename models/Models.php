...
/**
 * Table class for models
 */
class Models extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->Trademark->IsForeignKey = true; // Foreign key field
        $this->Trademark->Required = true; // Required field
        ...
        $this->Trademark->Lookup = new Lookup($this->Trademark, 'trademarks', false, 'ID', ["Trademark","","",""], '', '', [], [], [], [], [], [], false, '', '', "`Trademark`");
        $this->Trademark->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->Trademark->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->Model->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function getMasterFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "trademarks") {
            $masterTable = Container("trademarks");
            if ($this->Trademark->getSessionValue() != "") {
                $masterFilter .= "" . GetKeyFilter($masterTable->ID, $this->Trademark->getSessionValue(), $masterTable->ID->DataType, $masterTable->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getDetailFilterFromSession()
    {
        ...
        if ($this->getCurrentMasterTable() == "trademarks") {
            $masterTable = Container("trademarks");
            if ($this->Trademark->getSessionValue() != "") {
                $detailFilter .= "" . GetKeyFilter($this->Trademark, $this->Trademark->getSessionValue(), $masterTable->ID->DataType, $this->Dbid);
            } else {
                return "";
            }
        }
    ...
    public function getMasterFilter($masterTable, $keys)
    {
        ...
        switch ($masterTable->TableVar) {
            case "trademarks":
                ...
                if ($validKeys) {
                    return GetKeyFilter($masterTable->ID, $keys["Trademark"], $this->Trademark->DataType, $this->Dbid);
                }
                break;
        }
    ...
    public function getDetailFilter($masterTable)
    {
        switch ($masterTable->TableVar) {
            case "trademarks":
                return GetKeyFilter($this->Trademark, $masterTable->ID->DbValue, $masterTable->ID->DataType, $masterTable->Dbid);
        }
        return "";
    }
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "models";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("modelslist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "modelsview" => $Language->phrase("View"),
            "modelsedit" => $Language->phrase("Edit"),
            "modelsadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ModelsView",
            Config("API_ADD_ACTION") => "ModelsAdd",
            Config("API_EDIT_ACTION") => "ModelsEdit",
            Config("API_DELETE_ACTION") => "ModelsDelete",
            Config("API_LIST_ACTION") => "ModelsList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("modelsview", $parm);
        } else {
            $url = $this->keyUrl("modelsview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("modelsedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("modelslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("modelsadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("modelslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("modelsdelete", $parm);
        }
    }
    ...
    {
        if ($this->getCurrentMasterTable() == "trademarks" && !ContainsString($url, Config("TABLE_SHOW_MASTER") . "=")) {
            $url .= (ContainsString($url, "?") ? "&" : "?") . Config("TABLE_SHOW_MASTER") . "=" . $this->getCurrentMasterTable();
            $url .= "&" . GetForeignKeyUrl("fk_ID", $this->Trademark->getSessionValue()); // Use Session Value
        }
        return $url;
    }
