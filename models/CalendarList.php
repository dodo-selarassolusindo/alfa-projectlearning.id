 ...
 * Page class
 */
class CalendarList extends Calendar
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CalendarList";
    ...
    public $FormName = "fcalendarlist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "calendarlist";
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["calendar"]) || $GLOBALS["calendar"]::class == PROJECT_NAMESPACE . "calendar") {
        ...
        // Initialize URLs
        $this->AddUrl = "calendaradd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "calendardelete";
        $this->MultiUpdateUrl = "calendarupdate";
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "calendarview"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->AllDay->AdvancedSearch->toJson(), ","); // Field AllDay
        ...
        $filterList = Concat($filterList, $this->ClassNames->AdvancedSearch->toJson(), ","); // Field ClassNames
        $filterList = Concat($filterList, $this->Display->AdvancedSearch->toJson(), ","); // Field Display
        $filterList = Concat($filterList, $this->BackgroundColor->AdvancedSearch->toJson(), ","); // Field BackgroundColor
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fcalendarsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field AllDay
        $this->AllDay->AdvancedSearch->SearchValue = @$filter["x_AllDay"];
        $this->AllDay->AdvancedSearch->SearchOperator = @$filter["z_AllDay"];
        $this->AllDay->AdvancedSearch->SearchCondition = @$filter["v_AllDay"];
        $this->AllDay->AdvancedSearch->SearchValue2 = @$filter["y_AllDay"];
        $this->AllDay->AdvancedSearch->SearchOperator2 = @$filter["w_AllDay"];
        $this->AllDay->AdvancedSearch->save();
        ...
        // Field ClassNames
        $this->ClassNames->AdvancedSearch->SearchValue = @$filter["x_ClassNames"];
        $this->ClassNames->AdvancedSearch->SearchOperator = @$filter["z_ClassNames"];
        $this->ClassNames->AdvancedSearch->SearchCondition = @$filter["v_ClassNames"];
        $this->ClassNames->AdvancedSearch->SearchValue2 = @$filter["y_ClassNames"];
        $this->ClassNames->AdvancedSearch->SearchOperator2 = @$filter["w_ClassNames"];
        $this->ClassNames->AdvancedSearch->save();
        ...
        $this->BackgroundColor->AdvancedSearch->SearchValue = @$filter["x_BackgroundColor"];
        $this->BackgroundColor->AdvancedSearch->SearchOperator = @$filter["z_BackgroundColor"];
        $this->BackgroundColor->AdvancedSearch->SearchCondition = @$filter["v_BackgroundColor"];
        $this->BackgroundColor->AdvancedSearch->SearchValue2 = @$filter["y_BackgroundColor"];
        $this->BackgroundColor->AdvancedSearch->SearchOperator2 = @$filter["w_BackgroundColor"];
        $this->BackgroundColor->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        $searchFlds[] = &$this->GroupId;
        $searchFlds[] = &$this->Url;
        $searchFlds[] = &$this->ClassNames;
        $searchFlds[] = &$this->Display;
        $searchFlds[] = &$this->BackgroundColor;
        ...
        if ($this->CurrentMode == "view") {
            ...
            if ($Security->canView()) {
                if ($this->ModalView && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-view\" title=\"" . $viewcaption . "\" data-table=\"calendar\" data-caption=\"" . $viewcaption . "\" data-ew-action=\"modal\" data-action=\"view\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->ViewUrl)) . "\" data-btn=\"null\">" . $Language->phrase("ViewLink") . "</a>";
                } else {
            ...
            if ($Security->canEdit()) {
                if ($this->ModalEdit && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-edit\" title=\"" . $editcaption . "\" data-table=\"calendar\" data-caption=\"" . $editcaption . "\" data-ew-action=\"modal\" data-action=\"edit\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->EditUrl)) . "\" data-btn=\"SaveBtn\">" . $Language->phrase("EditLink") . "</a>";
                } else {
            ...
            if ($Security->canAdd()) {
                if ($this->ModalAdd && !IsMobile()) {
                    $opt->Body = "<a class=\"ew-row-link ew-copy\" title=\"" . $copycaption . "\" data-table=\"calendar\" data-caption=\"" . $copycaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->CopyUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("CopyLink") . "</a>";
                } else {
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcalendarlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fcalendarlist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->ModalAdd && !IsMobile()) {
            $item->Body = "<a class=\"ew-add-edit ew-add\" title=\"" . $addcaption . "\" data-table=\"calendar\" data-caption=\"" . $addcaption . "\" data-ew-action=\"modal\" data-action=\"add\" data-ajax=\"" . ($this->UseAjaxActions ? "true" : "false") . "\" data-url=\"" . HtmlEncode(GetUrl($this->AddUrl)) . "\" data-btn=\"AddBtn\">" . $Language->phrase("AddLink") . "</a>";
        } else {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "AllDay");
            $this->createColumnOption($option, "GroupId");
            $this->createColumnOption($option, "Url");
            $this->createColumnOption($option, "ClassNames");
            $this->createColumnOption($option, "Display");
            $this->createColumnOption($option, "BackgroundColor");
        }
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fcalendarlist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_calendar", "data-rowtype" => RowType::ADD]);
