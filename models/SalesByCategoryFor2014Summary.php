 ...
 * Page class
 */
class SalesByCategoryFor2014Summary extends SalesByCategoryFor2014
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SalesByCategoryFor2014Summary";
    ...
    public $CurrentPageName = "salesbycategoryfor2014";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'Sales_by_Category_for_2014';
        $this->TableName = 'Sales by Category for 2014';
        ...
        // Table object (Sales_by_Category_for_2014)
        if (!isset($GLOBALS["Sales_by_Category_for_2014"]) || $GLOBALS["Sales_by_Category_for_2014"]::class == PROJECT_NAMESPACE . "Sales_by_Category_for_2014") {
            $GLOBALS["Sales_by_Category_for_2014"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Sales by Category for 2014');
        }
    ...
    public function run()
    {
        ...
        $this->ProductSales->setVisibility();
    ...
    public function loadRowValues($record)
    {
        ...
        $data["ProductSales"] = $record['ProductSales'];
        $this->Rows[] = $data;
        $this->CategoryID->setDbValue($record['CategoryID']);
        $this->CategoryName->setDbValue(GroupValue($this->CategoryName, $record['CategoryName']));
        $this->ProductName->setDbValue($record['ProductName']);
        $this->ProductSales->setDbValue($record['ProductSales']);
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            $curVal = trim(strval($this->CategoryName->AdvancedSearch->SearchValue));
            if ($curVal != "") {
                $this->CategoryName->AdvancedSearch->ViewValue = $this->CategoryName->lookupCacheOption($curVal);
            } else {
                $this->CategoryName->AdvancedSearch->ViewValue = $this->CategoryName->Lookup !== null && is_array($this->CategoryName->lookupOptions()) && count($this->CategoryName->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->CategoryName->AdvancedSearch->ViewValue !== null) { // Load from cache
                $this->CategoryName->EditValue = array_values($this->CategoryName->lookupOptions());
            } else { // Lookup from database
                if ($curVal == "") {
                    $filterWrk = "0=1";
                } else {
                    $filterWrk = SearchFilter($this->CategoryName->Lookup->getTable()->Fields["CategoryName"]->searchExpression(), "=", $this->CategoryName->AdvancedSearch->SearchValue, $this->CategoryName->Lookup->getTable()->Fields["CategoryName"]->searchDataType(), "");
                }
                $sqlWrk = $this->CategoryName->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                $conn = Conn();
                ...
                $ari = count($rswrk);
                $arwrk = $rswrk;
                $this->CategoryName->EditValue = $arwrk;
            }
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->CategoryName->groupValue(); // Set up group attribute
            }

            // CategoryName
            $arwrk = [];
            $arwrk["lf"] = $this->CategoryName->CurrentValue;
            $arwrk["df"] = $this->CategoryName->CurrentValue;
            $arwrk = $this->CategoryName->Lookup->renderViewRow($arwrk, $this);
            $dispVal = $this->CategoryName->displayValue($arwrk);
            if ($dispVal != "") {
                $this->CategoryName->GroupViewValue = $dispVal;
            }
            ...
            // ProductSales
            $this->ProductSales->ViewValue = $this->ProductSales->CurrentValue;
            $this->ProductSales->ViewValue = FormatCurrency($this->ProductSales->ViewValue, $this->ProductSales->formatPattern());
            $this->ProductSales->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            // ProductSales
            $this->ProductSales->HrefValue = "";
            $this->ProductSales->TooltipValue = "";
        }
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            // ProductSales
            $currentValue = $this->ProductSales->CurrentValue;
            $viewValue = &$this->ProductSales->ViewValue;
            $viewAttrs = &$this->ProductSales->ViewAttrs;
            $cellAttrs = &$this->ProductSales->CellAttrs;
            $hrefValue = &$this->ProductSales->HrefValue;
            $linkAttrs = &$this->ProductSales->LinkAttrs;
            $this->cellRendered($this->ProductSales, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
        }
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fSales_by_Category_for_2014srch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    {
        return $this->CategoryName->Visible;
    }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fSales_by_Category_for_2014srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fSales_by_Category_for_2014srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    protected function getSort()
    {
        ...
        // Check for an Order parameter
            ...
            $this->updateSort($this->ProductSales); // ProductSales
    ...
    protected function getExtendedFilter()
    {
        ...
        if (Get("cmd") == "reset") {
            ...
            // Field CategoryName
            $this->getDropDownValue($this->CategoryName);
        ...
        if ($restoreSession) {
            $restoreDefault = true;
            if ($this->CategoryName->AdvancedSearch->issetSession()) { // Field CategoryName
        ...
        $this->buildDropDownFilter($this->CategoryName, $filter, false, true); // Field CategoryName
        $this->CategoryName->AdvancedSearch->save();

        // Field CategoryName
        LoadDropDownList($this->CategoryName->EditValue, $this->CategoryName->AdvancedSearch->SearchValue);
        return $filter;
    }
    ...
    {
        // Field CategoryName
        $this->CategoryName->AdvancedSearch->loadDefault();
    }
    ...
    public function showFilterList()
    {
        ...
        // Field CategoryName
        $extWrk = "";
        $this->buildDropDownFilter($this->CategoryName, $extWrk);
        $filter = "";
        if ($extWrk != "") {
    ...
    public function getFilterList()
    {
        ...
        // Field CategoryName
        $wrk = "";
        $wrk = $this->CategoryName->AdvancedSearch->SearchValue;
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
    ...
    protected function setupFilterList($filter)
    {
        ...
        // Field CategoryName
        if (!$this->CategoryName->AdvancedSearch->get($filter)) {
            $this->CategoryName->AdvancedSearch->loadDefault(); // Clear filter
        }
        $this->CategoryName->AdvancedSearch->save();
        return true;
    }
