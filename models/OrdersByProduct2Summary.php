 ...
 * Page class
 */
class OrdersByProduct2Summary extends OrdersByProduct2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "OrdersByProduct2Summary";
    ...
    public $CurrentPageName = "ordersbyproduct2";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'orders_by_product2';
        $this->TableName = 'orders_by_product2';
        ...
        // Table object (orders_by_product2)
        if (!isset($GLOBALS["orders_by_product2"]) || $GLOBALS["orders_by_product2"]::class == PROJECT_NAMESPACE . "orders_by_product2") {
            $GLOBALS["orders_by_product2"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'orders_by_product2');
        }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            $this->CompanyName->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
            ...
            $this->OrderDate->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");

            // Amount
            $this->Amount->ViewValue = $this->Amount->CurrentValue;
            $this->Amount->ViewValue = FormatCurrency($this->Amount->ViewValue, $this->Amount->formatPattern());
        ...
        if ($this->RowType == RowType::TOTAL) { // Summary row
            ...
            // CompanyName
            $currentValue = $this->CompanyName->CurrentValue;
            $viewValue = &$this->CompanyName->ViewValue;
            ...
            // OrderDate
            $currentValue = $this->OrderDate->CurrentValue;
            $viewValue = &$this->OrderDate->ViewValue;
            $viewAttrs = &$this->OrderDate->ViewAttrs;
            $cellAttrs = &$this->OrderDate->CellAttrs;
            $hrefValue = &$this->OrderDate->HrefValue;
            $linkAttrs = &$this->OrderDate->LinkAttrs;
            $this->cellRendered($this->OrderDate, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"forders_by_product2srch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"forders_by_product2srch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
    ...
    protected function getDrillDownFilter()
    {
        ...
        if ($opt == "1" || $opt == "2") {
            $mastertable = Param("s", ""); // Get source table
                $sql = Param("CategoryName", "");
                $sql = Decrypt($sql);
                $sql = str_replace("@CategoryName", "`CategoryName`", $sql);
                if ($sql != "") {
                    if ($filter != "") {
                        $filter .= " AND ";
                    }
                    $filter .= $sql;
                    if ($sql != "1=1") {
                        $filterList .= "<div><span class=\"ew-filter-caption\">" . $this->CategoryName->caption() . "</span><span class=\"ew-filter-value\">$sql</span></div>";
                    }
                }
                $sql = Param("OrderDate", "");
                $sql = Decrypt($sql);
                $sql = str_replace("@OrderDate", "`OrderDate`", $sql);
                if ($sql != "") {
                    if ($filter != "") {
                        $filter .= " AND ";
                    }
                    $filter .= $sql;
                    if ($sql != "1=1") {
                        $filterList .= "<div><span class=\"ew-filter-caption\">" . $this->OrderDate->caption() . "</span><span class=\"ew-filter-value\">$sql</span></div>";
                    }
                }
        }
        if ($opt == "1" || $opt == "2") {
