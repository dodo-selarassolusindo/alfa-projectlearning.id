 ...
 * Page class
 */
class SalesByYearSummary extends SalesByYear
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SalesByYearSummary";
    ...
    public $CurrentPageName = "salesbyyear";
    ...
    public function __construct()
    {
        ...
        // Table object (Sales_By_Year)
        if (!isset($GLOBALS["Sales_By_Year"]) || $GLOBALS["Sales_By_Year"]::class == PROJECT_NAMESPACE . "Sales_By_Year") {
            $GLOBALS["Sales_By_Year"] = &$this;
        }
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'Sales By Year');
        }
    ...
    public function loadGroupRowValues()
    {
        ...
        if ($this->GroupCount < $cnt) {
            $this->Year->setGroupValue(reset($this->GroupRecords[$this->GroupCount]));
        } else {
            $this->Year->setGroupValue("");
        }
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) {
            ...
            $rs = $sql->executeQuery();
            $records = $rs?->fetchAll() ?? [];
            $this->Subtotal->getSum($records, false);
            ...
            if ($rsagg) {
                $this->OrderID->Count = $this->TotalCount;
                $this->Subtotal->Count = $this->TotalCount;
                $this->Subtotal->SumValue = $rsagg["sum_subtotal"];
                $hasSummary = true;
            }
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->Subtotal->getSum($this->DetailRecords, false);
            }
        }
        ...
        if ($this->RowType == RowType::SEARCH) { // Search row
            ...
            if ($this->RowTotalType == RowSummary::GROUP) {
                $this->RowAttrs["data-group"] = $this->Year->groupValue(); // Set up group attribute
            }
            if ($this->RowTotalType == RowSummary::GROUP && $this->RowGroupLevel >= 2) {
                $this->RowAttrs["data-group-2"] = $this->ShippedDate->groupValue(); // Set up group attribute 2
            }

            // Year
            $this->Year->GroupViewValue = $this->Year->groupValue();
            $this->Year->CellCssClass = ($this->RowGroupLevel == 1 ? "ew-rpt-grp-summary-1" : "ew-rpt-grp-field-1");
            $this->Year->GroupViewValue = DisplayGroupValue($this->Year, $this->Year->GroupViewValue);

            // ShippedDate
            $this->ShippedDate->GroupViewValue = $this->ShippedDate->groupValue();
            $this->ShippedDate->CellCssClass = ($this->RowGroupLevel == 2 ? "ew-rpt-grp-summary-2" : "ew-rpt-grp-field-2");
            $this->ShippedDate->GroupViewValue = DisplayGroupValue($this->ShippedDate, $this->ShippedDate->GroupViewValue);

            // Subtotal
            $this->Subtotal->SumViewValue = $this->Subtotal->SumValue;
            $this->Subtotal->SumViewValue = FormatCurrency($this->Subtotal->SumViewValue, $this->Subtotal->formatPattern());
            $this->Subtotal->CellAttrs["class"] = ($this->RowTotalType == RowSummary::PAGE || $this->RowTotalType == RowSummary::GRAND) ? "ew-rpt-grp-aggregate" : "ew-rpt-grp-summary-" . $this->RowGroupLevel;
            ...
            // Year
            $this->Year->GroupViewValue = $this->Year->groupValue();
            $this->Year->CellCssClass = "ew-rpt-grp-field-1";
            $this->Year->GroupViewValue = DisplayGroupValue($this->Year, $this->Year->GroupViewValue);
            ...
            // ShippedDate
            $this->ShippedDate->GroupViewValue = $this->ShippedDate->groupValue();
            $this->ShippedDate->CellCssClass = "ew-rpt-grp-field-2";
            $this->ShippedDate->GroupViewValue = DisplayGroupValue($this->ShippedDate, $this->ShippedDate->GroupViewValue);
            if (!$this->ShippedDate->LevelBreak) {
                $this->ShippedDate->GroupViewValue = "";
            } else {
                $this->ShippedDate->LevelBreak = false;
            }
            ...
            $this->OrderID->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");

            // Subtotal
            $this->Subtotal->ViewValue = $this->Subtotal->CurrentValue;
            $this->Subtotal->ViewValue = FormatCurrency($this->Subtotal->ViewValue, $this->Subtotal->formatPattern());
            $this->Subtotal->CellCssClass = ($this->RecordCount % 2 != 1 ? "ew-table-alt-row" : "");
        ...
        if ($this->RowType == RowType::TOTAL) {
            ...
            $this->cellRendered($this->Year, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);

            // ShippedDate
            $currentValue = $this->ShippedDate->GroupViewValue;
            $viewValue = &$this->ShippedDate->GroupViewValue;
            $viewAttrs = &$this->ShippedDate->ViewAttrs;
            $cellAttrs = &$this->ShippedDate->CellAttrs;
            $hrefValue = &$this->ShippedDate->HrefValue;
            $linkAttrs = &$this->ShippedDate->LinkAttrs;
            $this->cellRendered($this->ShippedDate, $currentValue, $viewValue, $viewAttrs, $cellAttrs, $hrefValue, $linkAttrs);
            ...
            // ShippedDate
            $currentValue = $this->ShippedDate->groupValue();
            $viewValue = &$this->ShippedDate->GroupViewValue;
            $viewAttrs = &$this->ShippedDate->ViewAttrs;
            $cellAttrs = &$this->ShippedDate->CellAttrs;
            $hrefValue = &$this->ShippedDate->HrefValue;
            ...
            // OrderID
            $currentValue = $this->OrderID->CurrentValue;
            $viewValue = &$this->OrderID->ViewValue;
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fSales_By_Yearsrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = false;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fSales_By_Yearsrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = false;
