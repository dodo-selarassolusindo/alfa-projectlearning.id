...
class SalesByCustomer2Summary extends SalesByCustomer2
{
    ...
    public function __construct()
    {
        ...
        if (!isset($GLOBALS["Sales_By_Customer_2"]) || $GLOBALS["Sales_By_Customer_2"]::class == PROJECT_NAMESPACE . "Sales_By_Customer_2") {
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::TOTAL && $this->RowTotalSubType == RowTotal::FOOTER && $this->RowTotalType == RowSummary::PAGE) {
            ...
            $rs = $sql->executeQuery();
            $records = $rs?->fetchAll() ?? [];
            $this->Quantity->getSum($records, false);
            ...
            if (!$hasCount || !$hasSummary) {
                ...
                $this->Quantity->getSum($this->DetailRecords, false);
        ...
        if ($this->RowType == RowType::SEARCH) {
            ...
            if ($this->OrderID->AdvancedSearch->ViewValue !== null) { // Load from cache
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->OrderID->Lookup->getTable()->Fields["OrderID"]->searchExpression(), "=", $this->OrderID->AdvancedSearch->SearchValue, $this->OrderID->Lookup->getTable()->Fields["OrderID"]->searchDataType(), "");
                }
    ...
    public function getFilterList()
    {
        ...
        // Field OrderID
        $wrk = "";
        $wrk = $this->OrderID->AdvancedSearch->SearchValue;
        if (is_array($wrk)) {
            $wrk = implode("||", $wrk);
        }
        if ($wrk != "") {
