...
class CategoriesView extends Categories
{
    ...
    // Page object name
    public $PageObjName = "CategoriesView";
    ...
    // CSS class/style
    public $CurrentPageName = "categoriesview";
    ...
    public function __construct()
    {
        ...
        if (($keyValue = Get("CategoryID") ?? Route("CategoryID")) !== null) {
            $this->RecKey["CategoryID"] = $keyValue;
        }
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("CategoryID") ?? Route("CategoryID")) !== null) {
            ...
            $this->RecKey["CategoryID"] = $this->CategoryID->QueryStringValue;
        } elseif (Post("CategoryID") !== null) {
            $this->CategoryID->setFormValue(Post("CategoryID"));
            $this->RecKey["CategoryID"] = $this->CategoryID->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->CategoryID->setQueryStringValue($keyValue);
            $this->RecKey["CategoryID"] = $this->CategoryID->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "categorieslist"; // Return to list
        }
            ...
            case "show": // Get a record to display
                if (!$this->IsModal && !IsApi()) { // Normal view page
                    ...
                    if (!$matchRecord) {
                        ...
                        $returnUrl = "categorieslist"; // No matching record, return to list
                    } else {
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("productslist?" . Config("TABLE_SHOW_MASTER") . "=categories&" . GetForeignKeyUrl("fk_CategoryID", $this->CategoryID->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        $detailPageObj = Container("ProductsGrid");
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'categories')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=products"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
            $detailViewTblVar .= "products";
        }
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'categories')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=products"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            ...
            return '<button type="button" class="btn btn-default ew-export-link ew-email" title="' . $Language->phrase("ExportToEmail", true) . '" data-caption="' . $Language->phrase("ExportToEmail", true) . '" form="fcategoriesview" data-ew-action="email" data-hdr="' . $Language->phrase("ExportToEmail", true) . '" data-key="' . ArrayToJsonAttribute($this->RecKey) . '" data-exported-selected="false"' . $url . '>' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (products)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("products", explode(",", $this->getCurrentDetailTable()))) {
            $products = new ProductsList();
            $rsdetail = $products->loadRs($products->getDetailFilterFromSession(), $products->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $products->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
