...
/**
 * Table class for sales_by_category_for_2014
 */
class SalesByCategoryFor20142 extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "sales_by_category_for_20142";
        $this->TableName = 'sales_by_category_for_2014';
        $this->TableType = "VIEW";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "sales_by_category_for_2014";
        $this->Dbid = 'DB';
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->ProductSales->DbValue = $row['ProductSales'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("salesbycategoryfor20142list");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "salesbycategoryfor20142view" => $Language->phrase("View"),
            "salesbycategoryfor20142edit" => $Language->phrase("Edit"),
            "salesbycategoryfor20142add" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    {
        return "salesbycategoryfor20142list";
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "SalesByCategoryFor20142View",
            Config("API_ADD_ACTION") => "SalesByCategoryFor20142Add",
            Config("API_EDIT_ACTION") => "SalesByCategoryFor20142Edit",
            Config("API_DELETE_ACTION") => "SalesByCategoryFor20142Delete",
            Config("API_LIST_ACTION") => "SalesByCategoryFor20142List",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("salesbycategoryfor20142view", $parm);
        } else {
            $url = $this->keyUrl("salesbycategoryfor20142view", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "salesbycategoryfor20142add?" . $parm;
        } else {
            $url = "salesbycategoryfor20142add";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("salesbycategoryfor20142edit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("salesbycategoryfor20142list", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("salesbycategoryfor20142add", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("salesbycategoryfor20142list", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("salesbycategoryfor20142delete", $parm);
        }
    }
    ...
    public function loadListRowValues(&$rs)
    {
        ...
        $this->ProductSales->setDbValue($row['ProductSales']);
    }
    ...
    {
        global $Response;
        $listPage = "SalesByCategoryFor20142List";
    ...
    public function renderListRow()
    {
        ...
        $this->CategoryID->ViewValue = FormatNumber($this->CategoryID->ViewValue, $this->CategoryID->formatPattern());
        ...
        $this->ProductSales->ViewValue = FormatNumber($this->ProductSales->ViewValue, $this->ProductSales->formatPattern());
    ...
    public function renderEditRow()
    {
        ...
        if (strval($this->CategoryID->EditValue) != "" && is_numeric($this->CategoryID->EditValue)) {
            $this->CategoryID->EditValue = FormatNumber($this->CategoryID->EditValue, null);
        }
        ...
        // ProductSales
        $this->ProductSales->setupEditAttributes();
        $this->ProductSales->EditValue = $this->ProductSales->CurrentValue;
        $this->ProductSales->PlaceHolder = RemoveHtml($this->ProductSales->caption());
        if (strval($this->ProductSales->EditValue) != "" && is_numeric($this->ProductSales->EditValue)) {
            $this->ProductSales->EditValue = FormatNumber($this->ProductSales->EditValue, null);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->ProductSales);
                } else {
                    $doc->exportCaption($this->CategoryID);
                    $doc->exportCaption($this->CategoryName);
                    $doc->exportCaption($this->ProductName);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->ProductSales);
                    } else {
                        $doc->exportField($this->CategoryID);
                        $doc->exportField($this->CategoryName);
                        $doc->exportField($this->ProductName);
