...
class Orders2List extends Orders2
{
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("forders2srch", $filters);
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "RequiredDate");
            $this->createColumnOption($option, "ShippedDate");
            $this->createColumnOption($option, "Freight");
            $this->createColumnOption($option, "ShipAddress");
            $this->createColumnOption($option, "ShipCity");
            $this->createColumnOption($option, "ShipCountry");
        }
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_orders2", "data-rowtype" => RowType::ADD]);
        ...
        if ($this->isMultiEdit() && $this->RowType == RowType::EDIT && $this->EventCancelled) { // Update failed
            $this->restoreCurrentRowFormValues($this->RowIndex); // Restore form values
        }
    ...
    {
        $this->Freight->DefaultValue = $this->Freight->getDefault(); // PHP
        $this->Freight->OldValue = $this->Freight->DefaultValue;
    }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['OrderDate'])) { // OrderDate
            $this->OrderDate->setFormValue($row['OrderDate']);
        }
        if (isset($row['RequiredDate'])) { // RequiredDate
            $this->RequiredDate->setFormValue($row['RequiredDate']);
        }
        if (isset($row['ShippedDate'])) { // ShippedDate
            $this->ShippedDate->setFormValue($row['ShippedDate']);
        }
        if (isset($row['Freight'])) { // Freight
            $this->Freight->setFormValue($row['Freight']);
        }
        if (isset($row['ShipAddress'])) { // ShipAddress
            $this->ShipAddress->setFormValue($row['ShipAddress']);
        }
        if (isset($row['ShipCity'])) { // ShipCity
            $this->ShipCity->setFormValue($row['ShipCity']);
        }
        if (isset($row['ShipCountry'])) { // ShipCountry
            $this->ShipCountry->setFormValue($row['ShipCountry']);
        }
    }
