...
class SuppliersEdit extends Suppliers
{
    ...
    // CSS class/style
    public $CurrentPageName = "suppliersedit";
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "supplierslist") {
                            ...
                            $returnUrl = "supplierslist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function getEditRow($rsold)
    {
        ...
        // HomePage
        $this->HomePage->setDbValueDef($rsnew, $this->HomePage->CurrentValue, $this->HomePage->ReadOnly);
        return $rsnew;
    }
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['Region']); // Region
        $hash .= GetFieldHash($row['PostalCode']); // PostalCode
        $hash .= GetFieldHash($row['Country']); // Country
        $hash .= GetFieldHash($row['Phone']); // Phone
        $hash .= GetFieldHash($row['Fax']); // Fax
        $hash .= GetFieldHash($row['HomePage']); // HomePage
        return md5($hash);
    }
